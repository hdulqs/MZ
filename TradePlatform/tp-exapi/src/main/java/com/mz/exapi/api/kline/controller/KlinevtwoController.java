package com.mz.exapi.api.kline.controller;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.mz.core.constant.StringConstant;
import com.mz.exapi.api.kline.model.MarketDepths;
import com.mz.exapi.api.kline.model.MarketTrades;
import com.mz.exapi.api.kline.model.MarketTradesSub;
	 ...
	 /**
		...
		public HashMap<String, Object> data(HttpServletRequest request) {
			...
			if (!StringUtils.isEmpty(productListStr)) {
				List<String> productList = JSON.parseArray(productListStr, String.class);
				Map<String, List<MarketDetail>> marketDetail = marketDetail(request, productList);
				for (Entry<String, List<MarketDetail>> entry  : marketDetail.entrySet()) {
					if(entry.getKey().equals(coinCode)){
						map.put(coinCode, entry.getValue());
					}
				}
			}
...
//							data.put("transactionSum",transactionSum.setScale(4, BigDecimal.ROUND_HALF_DOWN));
	...
	// 首页数据推送
	@RequestMapping(value = "/indexshblw")
	@ResponseBody
	public List<JSONObject> indexshblw(HttpServletRequest request) {
		...
		if (!StringUtils.isEmpty(productListStr)) {
			...
			for (Coin coin : productList) {
				...
				if(!StringUtils.isEmpty(currentExchangPrice_str)){
					...
					data.put("currentExchangPrice",currentExchangPrice);
					if(currentExchangPrice.compareTo(BigDecimal.ZERO)>0){
						//System.out.println(coin.getCoinCode() + "_" + coin.getFixPriceCoinCode());
						
						if("USDT".equals(coin.getFixPriceCoinCode())){
							data.put("usdtcount", currentExchangPrice);
						}else{
							if("HSR".equals(coin.getCoinCode())&&"BTC".equals(coin.getFixPriceCoinCode())){
								System.out.println("来了");
							}
							//如果当前币对usdt有价格
							String usdtprice = redisService.get(coin.getFixPriceCoinCode() + "_USDT" +":currentExchangPrice");
							if(!StringUtils.isEmpty(usdtprice)){
								data.put("usdtcount", currentExchangPrice.multiply(new BigDecimal(usdtprice)));
							}else{
