...
public class Uploader {
	...
	public void upload() throws Exception {
		...
		try {
			...
			byte[] buff = new byte[bis.available()];//128
//			byte[]   data = new byte[this.inputStream.available()];  

			int count = -1;
			String aa="";
			while ((count = bis.read(buff)) != -1) {
//				aa+= Base64.encodeBase64String(buff);
			...
			//ccy 2018 1 18
			 File file = new File(this.getPhysicalPath(this.url)); 
		      ...
		      aa= Base64.encodeBase64String(buffer);
		
	//		String aa= Base64Img.GetImageStr(this.dizhi);
//			this.url=ToolUtil.getUploadPictures(aa);
			
			String Tname="NNLB";
		this.url=ToolUtil.getQiniu(aa, request, Tname);	
	...
	private CharacterEncoder BASE64Encoder() {
	...
	private String getPhysicalPath(String path) {
		String servletPath = this.request.getServletPath();//相对路径Tomcat
		String realPath = this.request.getSession().getServletContext()
				.getRealPath(servletPath);//绝对路径Tomcat
		//realPath="http://120.79.20.44:8080/image/";
