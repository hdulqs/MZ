...
public class LoginController extends BaseController {
			...
			if(null == code || "".equals(code)){
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){
					...
					if(pd != null){
						...
						session.setAttribute("UUUUUU", user);
					   ...
					   // UsernamePasswordToken token = new UsernamePasswordToken(USERNAME, PASSWORD); 
						...
						int a = ne.nextInt(9999-1000+1)+1000;//为变量赋随机值1000-9999
						
						if(Const.VERIFICATIONCODE ){
							
							a=1111;
							
						}else if( "18888899999".equals(user.getPHONE())){
							...
							Sms sms = new Sms();
							sms.sends(user.getPHONE(),a+"");
							
						}
						
						session.setAttribute("Smscode", a+"");
	...
	@ResponseBody
	public Object login_login2() throws Exception{
		Map<String,String> map = new HashMap<String,String>();
		PageData pd = new PageData();
		pd = this.getPageData();
		String errInfo = "";
		String code = pd.getString("smsCode");
		...
		String sessionCode=(String)session.getAttribute("Smscode");
			...
			if(null == code || "".equals(code)){
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){
						...
						pd.put("LAST_LOGIN",DateUtil.getTime().toString());
						userService.updateLastLogin(pd);
						User user = (User)session.getAttribute("UUUUUU");
						 
						session.setAttribute(Const.SESSION_USER, user);
						session.removeAttribute("Smscode");
						session.removeAttribute("UUUUUU");
						//shiro加入身份验证0
						Subject subject = SecurityUtils.getSubject(); 
	 ...
	 * 访问系统首页
	 */
	@RequestMapping(value="/main/verification")
	public ModelAndView verification(){
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		 
		mv.setViewName("system/admin/verification");
		mv.addObject("pd",pd);
		return mv;
	}
	...
	public ModelAndView tab1(){
		...
		try {
			if(Const.ISXJN){
				PageData pd = new PageData();
			PageData zcjh= pd;//xjn_minfoService.findhzsj(pd);//汇总数据-今日注册人数、昨日总业绩、未处理异常订单、未处理退单审核
				Subject currentUser = SecurityUtils.getSubject();  
				Session session = currentUser.getSession();
				User user = (User)session.getAttribute(Const.SESSION_USER);
				zcjh.put("QX", user.getNAME());
				mv.addObject("zcjh", zcjh);
				mv.setViewName("system/admin/default2");
			}else {
				PageData pd = new PageData();
				PageData zcjh=pd;// ttyc_minfoService.findzcjh(pd);//汇总数据-查询今日注册总人数、今日激活总人数、昨日总业绩
				Subject currentUser = SecurityUtils.getSubject();  
				Session session = currentUser.getSession();
				User user = (User)session.getAttribute(Const.SESSION_USER);
				zcjh.put("QX", user.getNAME());
