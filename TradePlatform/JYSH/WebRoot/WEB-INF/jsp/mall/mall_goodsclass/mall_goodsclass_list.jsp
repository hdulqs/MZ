...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="mall_goodsclass/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
							...
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${var.ID}" /><span class="lbl"></span></label>
										...
										<td>${var.CLASSNAME}</td>
										<td>
											<c:if test="${var.DISPLAY == true}">是</c:if>
											<c:if test="${var.DISPLAY == false}">否</c:if>
										</td>
										<td>${var.LEVEL}</td>
										<td>
											<c:if test="${var.RECOMMEND ==true}">是</c:if>
											<c:if test="${var.RECOMMEND ==false}">否</c:if>
										</td>
										<td>${var.SEQUENCE}</td>
										<td>${m.get(var.PARENT_ID)}</td>
										<td>
											<a href="<%=basePath%>${var.ICON_PATH}" class="bwGal"><img src="<%=basePath%>${var.ICON_PATH}" width="100"></a>
										</td>
								<td style="width: 30px;" class="center">
										...
										<div class='hidden-phone visible-desktop btn-group'>
											<c:if test="${QX.edit == 1 }"><a class='btn btn-mini btn-info' title="修改" onclick="edit('${var.ID}');"><i class='icon-edit'></i></a></c:if>
											<c:if test="${QX.del == 1 }"><a class='btn btn-mini btn-danger' title="删除" onclick="del('${var.ID}');"><i class='icon-trash'></i></a></c:if>
										</div>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>mall_goodsclass/goAdd.do';
			 diag.Width = 700;
			 diag.Height = 600;
				...
				if(confirm(msg)) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>mall_goodsclass/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>mall_goodsclass/excel.do';
		}
		</script>
		<style type="text/css">
