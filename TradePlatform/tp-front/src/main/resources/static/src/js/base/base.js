...
$(document).ajaxSuccess(function(event,xhr,options){
	if(xhr.getResponseHeader("nologin")=="true"){
		...
		if($.cookie('Language')=='' || $.cookie('Language')==null || $.cookie('Language')==undefined || $.cookie('Language')=='en'){
			layer.msg("Login timeout, please login again!",{ico:2},function(){
				window.location.href= loginUrl;
			})
		}else{
			layer.msg("登录超时",{ico:2},function(){
				window.location.href= loginUrl;
			})
		}
