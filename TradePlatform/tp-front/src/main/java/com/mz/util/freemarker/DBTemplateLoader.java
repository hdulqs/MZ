package com.mz.util.freemarker;
...
public class DBTemplateLoader implements TemplateLoader {
  ...
  public Object findTemplateSource(String name) throws IOException {
    try {
      ...
      String path = name.replace("_zh", "").replace("_zh_CN", "").replace("/", ":")
          .replace("_CN", "").replace("_en", "").replace("_en_US", "").replace("_US", "");
      //log.info(name+"----"+path);
      ...
      if (!StringUtils.isEmpty(ftl)) {
        ...
        return new StringTemplateSource(name, HtmlUtils.htmlUnescape(ftl),
