...
import com.mz.trade.entrust.ExEntrustSimple;
...
import com.mz.trade.entrust.service.ExEntrustMongoService;
...
public class ExEntrustServiceImpl extends BaseServiceImpl<ExEntrust, Long> implements
		ExEntrustService {
	...
	public String[] saveExEntrustTrade(EntrustTrade exEntrust) {
		...
		String[] aa=new String[]{exEntrust.getEntrustNum()};
	...
	public String[] saveExEntrust(ExEntrust exEntrust) {
		...
		LogFactory.info("委托初始化，耗时：" + (end - start));
		return relt;
	}
	...
	public String[] mqsaveExEntrust(ExEntrust exEntrust) {
		...
		long end = System.currentTimeMillis();
		LogFactory.info("委托mq中11，耗时：" + (end - start));
		if (relt[0].equals(CodeConstant.CODE_SUCCESS)) {
			long start1 = System.currentTimeMillis();
			LogFactory.info("委托单号：" + exEntrust.getEntrustNum());
			// 数据库保存委托单
			this.save(exEntrust);
			// 保存到redis中
			...
			LogFactory.info("委托mq中222，耗时：" + (end1 - start1));
			return relt;
		} else {
			return relt;
		}
	...
	public void saveExtrustToMongo(
			ExEntrust exEntrust) {/*
									 ...
									 * ExEntrust.class ,exEntrust .getWebsite(
									 * )+exEntrust .getCurrencyType() +exEntrust
									 * .getCoinCode ()+"_buy_entrust");
									 * mongoUtil .save(exEntrust); }else
									 * if(exEntrust.getType ()==2){
									 * MongoUtil<ExEntrust, Long> mongoUtil =
									 * new MongoUtil<ExEntrust, Long>(
									 * ExEntrust.class ,exEntrust .getWebsite(
									 * )+exEntrust .getCurrencyType() +exEntrust
									 * .getCoinCode ()+"_sell_entrust");
									 * mongoUtil .save(exEntrust); }
									 */
	}
	...
	/*	*//**
			 ...
			 *             exEntrust
			 * @return: void
			 * @Date : 2017年6月14日 下午3:38:32
			 * @throws:
			 *//*
			 * public void saveExtrustToRedis(ExEntrust exEntrust) { String
			 * table =this.getHeader(exEntrust); if (exEntrust.getType() == 1) {
			 * table = table + ":buy_entrust"; } else if (exEntrust.getType() ==
			 * 2) { table = table + ":sell_entrust"; } List<ExEntrust> list =
			 * JSON.parseArray(redisService.get(table), ExEntrust.class); if
			 * (list == null) { list = new ArrayList<ExEntrust>(); } list.add(0,
			 * exEntrust); redisService.save(table, JSON.toJSONString(list));
			 * 
			 * }
			 */
	 ...
	 *//*
		 * @Override public List<ExEntrust> listMatchBySellLimitedPrice(String
		 * saasId, ExEntrust sellentrust, String customerType) { String header
		 * =this.getHeader(sellentrust); String type =
		 * DifCustomer.getProductTransactionType(sellentrust.getCurrencyType());
		 * // 1普通，2坐市
		 * 
		 * String typeEntrust = ""; if (sellentrust.getType() == 1) {
		 * typeEntrust = "2"; } else if (sellentrust.getType() == 2) {
		 * typeEntrust = "1"; } QueryFilter filter = new
		 * QueryFilter(ExentrustOneday.class); filter.addFilter("type=",
		 * typeEntrust); filter.addFilter("coinCode=",
		 * sellentrust.getCoinCode()); filter.addFilter("fixPriceCoinCode=",
		 * sellentrust.getFixPriceCoinCode()); filter.addFilter("currencyType=",
		 * sellentrust.getCurrencyType()); filter.addFilter("website=",
		 * sellentrust.getWebsite()); filter.addFilter("customerType_in",
		 * customerType); filter.addFilter("entrustWay_in", "1,2");
		 * filter.addFilter("entrustPrice<=",
		 * sellentrust.getEntrustPrice().add(sellentrust.getFloatUpPrice()));
		 * filter.addFilter("entrustPrice>=",
		 * sellentrust.getEntrustPrice().subtract(sellentrust.getFloatDownPrice(
		 * )));
		 * 
		 * String onlyentrustNum = ExchangeDataCache.getStringData(header + ":"
		 * + ExchangeDataCache.OnlyentrustNum); String noMatchExEntrusts =
		 * ExchangeDataCache.getStringData(header + ":" +
		 * ExchangeDataCache.NoentrustNum); if (type.equals("2") &&
		 * !StringUtil.isEmpty(onlyentrustNum)) {// 做市
		 * filter.addFilter("entrustNum_in", onlyentrustNum); }
		 * 
		 * if (type.equals("2") && !StringUtil.isEmpty(noMatchExEntrusts)) {//
		 * 做市 List<String> listnoentrustNum = new ArrayList<String>(); if (null
		 * != noMatchExEntrusts) { String[] arry = noMatchExEntrusts.split(",");
		 * for (int a = 0; a < arry.length; a++) {
		 * listnoentrustNum.add(arry[a]); } }
		 * filter.addFilter("entrustNum_notin", noMatchExEntrusts);
		 * 
		 * } filter.setOrderby(
		 * "entrustWay desc,entrustPriceDouble desc,entrustTime asc");
		 * filter.setPage(0); filter.setPageSize(100); List<ExEntrust> entrusts
		 * = new ArrayList<ExEntrust>(); List<ExentrustOneday> list =
		 * exentrustOnedayService.find(filter); if (list != null) { for
		 * (ExentrustOneday l : list) { String entrust = JSON.toJSONString(l);
		 * entrusts.add(JSON.parseObject(entrust, ExEntrust.class)); } } return
		 * entrusts;
		 * 
		 * }
		 */
	...
	@Override
	public List<ExOrderInfo> getMatchDetail(String entrustNum, String coinCode) {
		String saasId = PropertiesUtils.APP.getProperty("app.saasId");
	...
	public String[] cancelExEntrustnokey(ExEntrust exEntrust,String remark) {
		...
		if (StringUtil.isEmpty(entrustredis) || entrustredis.equals("[]")) {
			...
			if(i==0){//没找到在key值里面
				LogFactory.info("撤销失败，此委托单已不在key值里面" + key+"==exEntrust.getEntrustNum()");
				flag=true;
			}
		}
	...
	public String[] mqcancelExEntrust(ExEntrust entrust) {
			...
			if (exEntrust.getType().equals(1)) {
				...
				if (exEntrust.getFixPriceType().equals(0)) {
					...
					if (relt[0].equals(CodeConstant.CODE_FAILED)) {
						LogFactory.info(remark + "--撤销 出现异常(" + relt[1] + ")：entrustNum===" + exEntrust.getEntrustNum() + "，时间==" + DateUtil.dateToString(new Date(), StringConstant.DATE_FORMAT_FULL));
						System.out.println(remark + "--撤销 出现异常(" + relt[1] + ")：entrustNum===" + exEntrust.getEntrustNum() + "，时间==" + DateUtil.dateToString(new Date(), StringConstant.DATE_FORMAT_FULL));
					}
		...
		if (relt[0].equals(CodeConstant.CODE_SUCCESS)) {
			...
			LogFactory.info(remark + "撤销 成功->entrustNum:" + exEntrust.getEntrustNum() + ",userName:" + exEntrust.getUserName());
		} else {
			LogFactory.info(remark + "撤销 失败->entrustNum:" + exEntrust.getEntrustNum() + ",userName:" + exEntrust.getUserName());
		}
		relt[0] = CodeConstant.CODE_SUCCESS;
		relt[1] = "";
		return relt;
	...
	public String[] saveExEntrustCheck(EntrustTrade exEntrust) {
		...
		Coin productCommon = productCommonService.getProductCommon(exEntrust.getCoinCode(), exEntrust.getFixPriceCoinCode());
		//start 交易币——定价币是否开启操盘，用户是否操盘账户 2018.5.15 gt
		QueryFilter f = new QueryFilter(AppPersonInfo.class);
		f.addFilter("customerId=", exEntrust.getCustomerId());
		AppPersonInfo personInfo = appPersonInfoService.get(f);

		QueryFilter c = new QueryFilter(ExCointoCoin.class);
		c.addFilter("coinCode=", exEntrust.getCoinCode());
		c.addFilter("fixPriceCoinCode=", exEntrust.getFixPriceCoinCode());
		ExCointoCoin exCointoCoin = exCointoCoinService.get(c);
		if(personInfo.getCustomerType()!=2 && exCointoCoin.getIsOperate()==1){
			rt[1] = exEntrust.getCoinCode() + "_" + exEntrust.getFixPriceCoinCode() + "::cannot_trade";
			return rt;
		}
		//end 交易币——定价币是否开启操盘，用户是否操盘账户 2018.5.15 gt
		...
		if (exEntrust.getEntrustWay() == 1 && productCommon != null) {
			if ( productCommon.getRose()!=null
					...
					&& !StringUtil.isEmpty(productCommon.getYesterdayPrice())
					) {
				BigDecimal rose = productCommon.getRose();// 涨幅
				BigDecimal zhangjia = new BigDecimal(productCommon.getYesterdayPrice()).multiply((new BigDecimal("100").add(rose)).divide(new BigDecimal(100),8,BigDecimal.ROUND_HALF_DOWN));
			...
			if ( productCommon.getDecline()!=null
				...
				BigDecimal diejia = new BigDecimal(productCommon.getYesterdayPrice()).multiply((new BigDecimal("100").subtract(rose)).divide(new BigDecimal(100),8,BigDecimal.ROUND_HALF_DOWN));
