...
import com.mz.trade.websoketContext.model.MarketTrades;
import com.mz.trade.websoketContext.model.MarketTradesSelf;
import com.mz.trade.websoketContext.model.MarketTradesSub;
import com.mz.trade.websoketContext.model.Total;
...
public class ExOrderServiceImpl extends BaseServiceImpl<ExOrder, Long>
  ...
  public void setCurrentExchangPrice(String header) {
    ...
    if (null != list && list.size() != 0) {
      ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.CurrentExchangPrice,
          list.get(0).getTransactionPrice().toString());
      ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.CurrentExchangDate,
          DateUtil.dateToString(list.get(0).getTransactionTime(), StringConstant.DATE_FORMAT_YMD)
              .toString());
      ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.CurrentExchangTime,
          DateUtil.dateToString(list.get(0).getTransactionTime(), StringConstant.DATE_FORMAT_FULL)
              .toString());
    } else {
      ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.CurrentExchangPrice, "0.00");
      ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.CurrentExchangDate,
          DateUtil.dateToString(new Date(), StringConstant.DATE_FORMAT_YMD).toString());
      ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.CurrentExchangTime,
          DateUtil.dateToString(new Date(), StringConstant.DATE_FORMAT_FULL).toString());
    }
  }
    ...
    if (null != list && list.size() != 0) {
      if (list.size() == 2) {
        ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.LastExchangPrice,
            list.get(1).getTransactionPrice().toString());
      } else if (list.size() == 1) {
        ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.LastExchangPrice,
            list.get(0).getTransactionPrice().toString());
      }
  ...
  public void setOpenedExchangPrice(String header, BigDecimal issuePrice) {
    ...
    if (null != list && list.size() != 0) {
      ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.OpenedExchangPrice,
          list.get(0).getTransactionPrice().toString());
    } else {
      ...
      if (null != list1 && list1.size() != 0) {
        ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.OpenedExchangPrice,
            list1.get(0).getTransactionPrice().toString());
      } else {
        ExchangeDataCache.setStringData(header + ":" + ExchangeDataCache.OpenedExchangPrice,
            issuePrice == null ? "0.00" : issuePrice.toString());
      }
    }
  }
  ...
  public String getNewList(String header, Integer count) {
    ...
    SimplePropertyPreFilter s = new SimplePropertyPreFilter(ExOrder.class, "coinCode",
        "transactionTime", "transactionPrice", "transactionCount");
  ...
  public String getNewListMarket(String header, Integer count, String timeorder) {
    ...
    String[] headarry = header.split(":");
    Coin productCommon = productCommonService
        .getProductCommon(headarry[2].split("_")[0], headarry[2].split("_")[1]);
    int keepDecimalForCoin = 4;
    int keepDecimalForCurrency = 2;
    if (null != productCommon) {
      keepDecimalForCoin = (null != productCommon.getKeepDecimalForCoin() ? productCommon
          .getKeepDecimalForCoin() : 4);
      keepDecimalForCurrency = (null != productCommon.getKeepDecimalForCurrency() ? productCommon
          .getKeepDecimalForCurrency() : 2);
    }
    if (null == count) {
      count = 10;
    }
    ...
    if (timeorder.equals("asc")) {
      ...
      if (DifCustomer.isexOrderIsMemory()) {
        ...
        list = redisService.getList1(header + ":" + ExchangeDataCache.LastOrderRecords);
    ...
    for (ExOrder l : list) {
      ...
      sub.setPrice(
          l.getTransactionPrice().setScale(keepDecimalForCurrency, BigDecimal.ROUND_HALF_UP));
  ...
  public String findNewListMarketnewAdd(String header, String minDate, String maxDate) {
    ...
    if (DifCustomer.isexOrderIsMemory()) {
      list = redisService.getList1(header + ":" + ExchangeDataCache.LastOrderRecordAdds);
      if (null == list || list.size() == 0) {
        ...
        Collections.reverse(list);   //倒序
        //取一次就清空一次，所以取得就是增量
        redisService.setList(header + ":" + ExchangeDataCache.LastOrderRecordAdds,
            new ArrayList<ExOrder>());
      }
  ...
  public void pushTotal(String coinCode) {
    StringBuffer sb = new StringBuffer(
        "{\"coinCode\":\"" + coinCode + "\",\"data\":" + getTotal(coinCode) + "}");
  ...
  public String getTotal(String header) {
    ...
    currentExchangPrice = (null != currentExchangPrice ? currentExchangPrice
        : new BigDecimal("0.00").toString());

    //最新成交价上一个
    String lastExchangPrice = ExchangeDataCache
        .getStringData(header + ":" + ExchangeDataCache.LastExchangPrice);
    lastExchangPrice = (null != lastExchangPrice ? lastExchangPrice
        : new BigDecimal("0.00").toString());
    total.setCurrentExchangPrice(new BigDecimal(currentExchangPrice)
        .setScale(keepDecimalForCurrency, BigDecimal.ROUND_HALF_EVEN).toString());
    total.setLastExchangPrice(new BigDecimal(lastExchangPrice)
        .setScale(keepDecimalForCurrency, BigDecimal.ROUND_HALF_EVEN).toString());
    //开盘价
    String openedExchangPrice = ExchangeDataCache
        .getStringData(header + ":" + ExchangeDataCache.OpenedExchangPrice);
    openedExchangPrice = (null != openedExchangPrice ? openedExchangPrice
        : new BigDecimal("0.00").toString());

    //日跌涨幅
    BigDecimal riseAndFall = new BigDecimal("0.00");
    if (openedExchangPrice != null && (new BigDecimal(openedExchangPrice).compareTo(BigDecimal.ZERO)
        > 0)) {
      BigDecimal fenmu = BigDecimal.ONE;
      String table =
          "TransactionOrder_" + headarry[0] + "_" + headarry[1] + "_" + headarry[2] + "_1440";
      List<TransactionOrder> list = JSON
      ...
      if (list != null && list.size() > 0) {
        ...
        if (fenmu.compareTo(BigDecimal.ZERO) != 0) {
          BigDecimal bigDecimal = (new BigDecimal(currentExchangPrice).subtract(fenmu))
              .multiply(new BigDecimal(100)).divide(fenmu, 2);
    ...
    seramap.put("fixPriceCoinCode", headarry[2].split("_")[1]);
    seramap.put("minDate",
        DateUtil.dateToString(DateUtil.addDay(new Date(), -1), "yyyy-MM-dd HH:mm:ss"));
    ...
    if (null != list && list.size() != 0) {
      ...
      if (null != o) {
        total.setMaxPrice(
            o.getMaxPrice().setScale(keepDecimalForCurrency, BigDecimal.ROUND_HALF_EVEN)
                .toString());
        total.setMinPrice(
            o.getMinPrice().setScale(keepDecimalForCurrency, BigDecimal.ROUND_HALF_EVEN)
                .toString());
        total.setTransactionCount(
            o.getTransactionCount().setScale(keepDecimalForCoin, BigDecimal.ROUND_HALF_EVEN)
                .toString());
        total.setTransactionSum(null == o.getAgvPrice() ? "0.00"
            : o.getAgvPrice().setScale(keepDecimalForCoin, BigDecimal.ROUND_HALF_EVEN).toString());
      } else {
