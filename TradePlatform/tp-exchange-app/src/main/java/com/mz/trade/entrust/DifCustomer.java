...
public class DifCustomer {
  ...
  public static String differetCustomer = PropertiesUtils.APP
      .getProperty("app.differetCustomer");//做成配置文件,如果是坐市商的话就用theSeat
  public static String exEntrustIsUseMongo = PropertiesUtils.APP
      .getProperty("app.exEntrustIsUseMongo");
  //是否开启闭盘定时器
  ...
  public static Boolean isInOpenplateAndCloseplate() {
    ...
    openAndclosePlateTime =
        StringUtil.isEmpty(openAndclosePlateTime) ? "09:00:00,12:00:00,14:00:00,23:00:00"
            : openAndclosePlateTime; //
    if (!StringUtil.isEmpty(openAndclosePlateTime)) {
      ...
      while (i < arrtime.length) {
        ...
        Integer sumnowdatearr =
            Integer.valueOf(nowdatearr[0]) * 3600 + Integer.valueOf(nowdatearr[1]) * 60 + Integer
                .valueOf(nowdatearr[2]);

        String[] openingTimearr = arrtime[i].split(":");
        Integer sumopeningTimearr =
            Integer.valueOf(openingTimearr[0]) * 3600 + Integer.valueOf(openingTimearr[1]) * 60
                + Integer.valueOf(openingTimearr[2]);

        String[] closeTimearr = arrtime[i + 1].split(":");
        Integer sumcloseTimearr =
            Integer.valueOf(closeTimearr[0]) * 3600 + Integer.valueOf(closeTimearr[1]) * 60
                + Integer.valueOf(closeTimearr[2]);
  ...
  public static Boolean isInOpenAndCloseplate() {
    ...
    if (openingTime.equals(closeTime)) {
      ...
      Integer sumcloseTimearr =
          Integer.valueOf(closeTimearr[0]) * 3600 + Integer.valueOf(closeTimearr[1]) * 60 + Integer
              .valueOf(closeTimearr[2]);
  ...
  public static Boolean isInClosePlateAndClose() {
    ...
    Integer sumclosePlateTime =
        Integer.valueOf(closePlateTimearr[0]) * 3600 + Integer.valueOf(closePlateTimearr[1]) * 60
            + Integer.valueOf(closePlateTimearr[2]);
