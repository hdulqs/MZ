...
import com.mz.exchange.remote.subscription.RemoteExSubscriptionService;
...
public class ExDigitalmoneyAccountServiceImpl extends
  ...
  @Override
  public ExDigitalmoneyAccount getByCustomerIdAndType(Long customerId, String coinCode,
   ...
   */
  @Override
  public List<ExDigitalmoneyAccount> getByCustomerId(Long customerId, String currencyType,
      String website) {
    QueryFilter filter = new QueryFilter(ExDigitalmoneyAccount.class);
    filter.addFilter("customerId=", customerId);
    filter.addFilter("currencyType=", currencyType);
...
/*		String[] relt=new String[2];  
		...
		if(a==0){
			relt[0]=CodeConstant.CODE_FAILED;
			relt[1]="并发出现异常";
			LogFactory.info("并发出现异常：exdmaccountid===" + account.getId()+ "，时间=="+DateUtil.dateToString(new Date(), StringConstant.DATE_FORMAT_FULL));
			System.out.println("并发出现异常：exdmaccountid===" + account.getId()+ "，时间=="+DateUtil.dateToString(new Date(), StringConstant.DATE_FORMAT_FULL));
			return relt;
  ...
  public String[] updateAccount1(ExDigitalmoneyAccount account, BigDecimal freezeMoney, int n) {
    ...
    String[] relt = new String[2];
    ExDigitalmoneyAccountDao exDigitalmoneyAccountDao = (ExDigitalmoneyAccountDao) dao;
    int a = exDigitalmoneyAccountDao
        .updateByVersion(account.getColdMoney(), account.getHotMoney(), account.getLendMoney(),
            account.getVersion() + 1, account.getCustomerId(), account.getCoinCode(),
            account.getCurrencyType(), account.getWebsite(), account.getVersion());
    //	System.out.println("a===="+a);
    System.out.println("account.getVersion()====" + account.getVersion());
    if (a == 0) {
      //热钱包的金额小于冻结金额则失败
      if (account.getHotMoney().compareTo(freezeMoney) < 0) {
        ...
        if (n == 10) {
          relt[0] = CodeConstant.CODE_FAILED;
          relt[1] = "并发出现异常";
          System.out.println("relt【0】====" + relt[0]);
  ...
  @Override
  public String[] freezeAccountSelf(Long id, BigDecimal freezeMoney, String transactionNum,
    ...
    if (relt[0].equals(CodeConstant.CODE_SUCCESS)) {
      //冷钱包增加一条收入的记录
      ExDmColdAccountRecord coldAccountRecord = createColdRecord(1, account, freezeMoney,
          transactionNum, remark);
      //热钱包增加一条支出的记录
      ExDmHotAccountRecord hotAccountRecord = createHotRecord(2, account, freezeMoney,
          transactionNum, remark);

      if (null == isImmediatelySaveRecord || (null != isImmediatelySaveRecord
          && isImmediatelySaveRecord == 1)) {
  ...
  @Override
  public String[] unfreezeAccountSelf(Long id, BigDecimal unfreezeMoney, String transactionNum,
      String remark, Integer isculAppAccount, Integer isImmediatelySaveRecord) {
    String[] relt = new String[2];
    ExDigitalmoneyAccount account = this.get(id);
    relt = commonret(account);
    ...
    if (relt[0].equals(CodeConstant.CODE_SUCCESS)) {
      //冷钱包增加一条支出的记录
      ExDmColdAccountRecord coldAccountRecord = createColdRecord(2, account, unfreezeMoney,
          transactionNum, remark);
      //热钱包增加一条收入的记录
      ExDmHotAccountRecord hotAccountRecord = createHotRecord(1, account, unfreezeMoney,
          transactionNum, remark);
      if (null == isImmediatelySaveRecord || (null != isImmediatelySaveRecord
          && isImmediatelySaveRecord == 1)) {
  ...
  @Override
  public String[] unfreezeAccountThem(Long id, BigDecimal unfreezeMoney, String transactionNum,
      String remark, Integer isculAppAccount, Integer isImmediatelySaveRecord) {
    String[] relt = new String[2];
    ExDigitalmoneyAccount account = this.get(id);
    relt = commonret(account);
   ...
   */
  @Override
  public String[] inComeToHotAccount(Long id, BigDecimal incomeMoney, String transactionNum,
      String remark, Integer isculAppAccount, Integer isImmediatelySaveRecord) {
    String[] relt = new String[2];
    ExDigitalmoneyAccount account = this.get(id);
    relt = commonret(account);
    ...
    if (relt[0].equals(CodeConstant.CODE_SUCCESS)) {
      //热钱包增加一条收入的记录
      ExDmHotAccountRecord hotAccountRecord = createHotRecord(1, account, incomeMoney,
          transactionNum, remark);
      if (null == isImmediatelySaveRecord || (null != isImmediatelySaveRecord
          && isImmediatelySaveRecord == 1)) {
        exDmHotAccountRecordService.save(hotAccountRecord);
  ...
  @Override
  public String[] payFromHotAccount(Long id, BigDecimal payMoney, String transactionNum,
      String remark, Integer isculAppAccount, Integer isImmediatelySaveRecord) {
    String[] relt = new String[2];
    ExDigitalmoneyAccount account = this.get(id);
    relt = commonret(account);
    ...
    if (relt[0].equals(CodeConstant.CODE_SUCCESS)) {
      //热钱包增加一条减少的记录
      ExDmHotAccountRecord hotAccountRecord = createHotRecord(2, account, payMoney, transactionNum,
          remark);
      if (null == isImmediatelySaveRecord || (null != isImmediatelySaveRecord
          && isImmediatelySaveRecord == 1)) {
        exDmHotAccountRecordService.save(hotAccountRecord);
  ...
  @Override
  public String[] payFromHotAccountNegative(Long id, BigDecimal payMoney, String transactionNum,
      String remark, Integer isculAppAccount, Integer isImmediatelySaveRecord) {
    String[] relt = new String[2];
    ExDigitalmoneyAccount account = this.get(id);
    relt = commonret(account);
  ...
  public ExDmColdAccountRecord createColdRecord(Integer recordType,
      ExDigitalmoneyAccount exDigitalmoneyAccount, BigDecimal freezeMoney, String transactionNum,
      String remark) {
  ...
  public ExDmHotAccountRecord createHotRecord(Integer recordType, ExDigitalmoneyAccount account,
      BigDecimal freezeMoney, String transactionNum, String remark) {
    //资金池5(完成)，托管0(处理中)将来通过配置文件来获取判断
    Integer status = 5;
  ...
  @Override
  public HashMap<String, BigDecimal> getAllNetAsset(Long customerId, String currencyType,
      String website) {
    ...
    for (ExDigitalmoneyAccount l : list) {
      ...
      BigDecimal currentExchangPrice = new BigDecimal(ExchangeDataCache.getStringData(
          l.getWebsite() + ":" + l.getCurrencyType() + ":" + l.getCoinCode() + ":"
              + ExchangeDataCache.CurrentExchangPrice));
      if (null != currentExchangPrice && currentExchangPrice.compareTo(new BigDecimal("0")) != 0) {
        coinAccountNetAsse = rMBappAccountNetAsse
            .divide(currentExchangPrice, 4, BigDecimal.ROUND_HALF_DOWN);
  ...
  @Override
  public HashMap<String, BigDecimal> getByTypeNetAsset(Long customerId, String coinCode,
      String currencyType, String website) {
    HashMap<String, BigDecimal> map = new HashMap<String, BigDecimal>();
    BigDecimal rMBappAccountNetAsse = getRMBNetAsset(customerId, currencyType, website);
    map.put("RMBAccountNetAsse", rMBappAccountNetAsse);
    ...
    BigDecimal coinAccountNetAsse = new BigDecimal("0");
    String stringData = ExchangeDataCache.getStringData(
        website + ":" + currencyType + ":" + coinCode + ":"
            + ExchangeDataCache.CurrentExchangPrice);
  ...
  public BigDecimal getRMBNetAsset(Long customerId, String currencyType, String website) {
    ...
    AppAccount appAccount = remoteAppAccountService
        .getByCustomerIdAndType(customerId, currencyType, website);
    if (appAccount != null) {
      ...
      for (ExDigitalmoneyAccount l : list) {
        ...
        String stringData = ExchangeDataCache.getStringData(
            l.getWebsite() + ":" + l.getCurrencyType() + ":" + l.getCoinCode() + "_" + upperCase
                + ":" + ExchangeDataCache.CurrentExchangPrice);
        System.out.println(
            "let=========" + l.getWebsite() + ":" + l.getCurrencyType() + ":" + l.getCoinCode()
                + "_" + upperCase + ":" + ExchangeDataCache.CurrentExchangPrice);
        if (!StringUtils.isEmpty(stringData)) {
          BigDecimal currentExchangPrice = new BigDecimal(ExchangeDataCache.getStringData(
              l.getWebsite() + ":" + l.getCurrencyType() + ":" + l.getCoinCode() + "_" + upperCase
                  + ":" + ExchangeDataCache.CurrentExchangPrice));
  ...
  @Override
  public BigDecimal getCoinNetAsset(Long customerId, String coinCode, String currencyType,
      String website) {
    BigDecimal rMBappAccountNetAsse = getRMBNetAsset(customerId, currencyType, website);
    BigDecimal currentExchangPrice = new BigDecimal(ExchangeDataCache.getStringData(
        website + ":" + currencyType + ":" + coinCode + ":"
  ...
  public BigDecimal getRMBLendMoneyInteretSum(Long customerId, String currencyType,
      String website) {
    String saasId = PropertiesUtils.APP.getProperty("app.saasId");
    RpcContext.getContext().setAttachment("saasId", saasId);
    RemoteAppAccountService remoteAppAccountService = (RemoteAppAccountService) ContextUtil
    ...
    if (appAccount != null) {
      ...
      ExDmLend exDmLend = exDmLendService
          .getLendBycustmer(currencyType, customerId, currencyType, website);
      lendSum = lendSum.add(exDmLend.getNotrepayInterestCount());
      for (ExDigitalmoneyAccount l : list) {
        BigDecimal a = new BigDecimal("0");
        //加未偿还的利息
        exDmLend = exDmLendService
            .getLendBycustmer(l.getCoinCode(), customerId, currencyType, website);
        a = a.add(exDmLend.getNotrepayInterestCount());
        String stringData = ExchangeDataCache.getStringData(
            l.getWebsite() + ":" + l.getCurrencyType() + ":" + l.getCoinCode() + ":"
                + ExchangeDataCache.CurrentExchangPrice);
  ...
  @Override
  public HashMap<String, BigDecimal> getByAllInteret(Long customerId, String coinCode,
      String currencyType, String website) {
    HashMap<String, BigDecimal> map = new HashMap<String, BigDecimal>();
    ExDmLend exDmLend = exDmLendService
        .getLendBycustmer(currencyType, customerId, currencyType, website);
    ...
    List<ExDigitalmoneyAccount> list = this.getByCustomerId(customerId, currencyType, website);
    for (ExDigitalmoneyAccount l : list) {
      ExDmLend exDmLend1 = exDmLendService
          .getLendBycustmer(currencyType, customerId, currencyType, website);
  ...
  @Override
  public BigDecimal getCanLendMoney(Long customerId, String coinCode, String currencyType,
      String website) {
    ...
    BigDecimal lendleastMoney = new BigDecimal(
        StringUtil.isEmpty(lendleastMoneys) ? "1000" : lendleastMoneys); //杠杆的倍数，做成配置文件
    ...
    BigDecimal times = new BigDecimal(StringUtil.isEmpty(lendTimes) ? "3" : lendTimes)
        .subtract(new BigDecimal("1")); //杠杆的倍数，做成配置文件
    ...
    BigDecimal currentExchangPrice = new BigDecimal(ExchangeDataCache.getStringData(
        website + ":" + currencyType + ":" + coinCode + ":"
            + ExchangeDataCache.CurrentExchangPrice));
    BigDecimal canLendMoney = surMBAccountNetAsse
        .divide(currentExchangPrice, 4, BigDecimal.ROUND_HALF_DOWN);
  ...
  @Override
  public HashMap<String, BigDecimal> getAllCanLendMoney(Long customerId, String coinCode,
      String currencyType, String website) {
    HashMap<String, BigDecimal> map = new HashMap<String, BigDecimal>();
  ...
  @Override
  public HashMap<String, BigDecimal> getAllLendMoney(Long customerId, String currencyType,
      String website) {
    HashMap<String, BigDecimal> map = new HashMap<String, BigDecimal>();
    String saasId = PropertiesUtils.APP.getProperty("app.saasId");
    RpcContext.getContext().setAttachment("saasId", saasId);
   ...
   */
  @Override
  public BigDecimal getCanWithdrawMoney(Long customerId, String coinCode, String currencyType,
      String website) {
    RemoteAppAccountService remoteAppAccountService = (RemoteAppAccountService) ContextUtil
        .getBean("remoteAppAccountService");
    AppAccount appAccount = remoteAppAccountService
    ...
    BigDecimal lengPing = new BigDecimal(
        StringUtil.isEmpty(lengPings) ? "110" : lengPings); //杠杆的倍数，做成配置文件
   ...
   */
  @Override
  public BigDecimal getCanWithdrawCoin(Long customerId, String coinCode, String currencyType,
      String website) {

    ExDigitalmoneyAccount eda = this
        .getByCustomerIdAndType(customerId, coinCode, currencyType, website);
    ...
    BigDecimal coinNetAsset = this
        .getCoinNetAsset(customerId, coinCode, currencyType, website); //净资产
  ...
  public BigDecimal netAsseToLend(Long customerId, String currencyType, String website) {
    ...
    return sumzichan.divide(rmblendSum, 4, BigDecimal.ROUND_HALF_DOWN)
  ...
  public List<DigitalmoneyAccountAndProduct> findDigitalAndProduct(String website,
    ...
    ExDigitalmoneyAccountDao exDigitalmoneyAccountDao = (ExDigitalmoneyAccountDao) dao;
    List<DigitalmoneyAccountAndProduct> findNewProductByCustomer = exDigitalmoneyAccountDao
        .findNewProductByCustomer(website, cutomerName, isMarket);
  ...
  public PageResult findPageBySql(QueryFilter filter) {
    ...
    String publicKey = filter.getRequest().getParameter("publicKey_like");
  ...
  public JsonResult disableMoney(Long id, String disableMoey) {
    ...
    ExDigitalmoneyAccount account = this.get(id);
    if (account.getHotMoney().compareTo(new BigDecimal(disableMoey).add(account.getDisableMoney()))
        >= 0) {
   ...
   */
  @Override
  public JsonResult coinPoking(Long id, String number) {
    //查询认购期内的计划
    RemoteExSubscriptionService remoteExSubscriptionService = (RemoteExSubscriptionService) ContextUtil
        .getBean("remoteExSubscriptionService");
    remoteExSubscriptionService.exPlanList("2");
    return null;
  }

  @Override
  public String[] unfreezeAccountSelfCancelExEntrust(Long id, BigDecimal unfreezeMoney,
    ...
    if (account.getColdMoney().compareTo(unfreezeMoney) < 0) {
			...
			return relt;*/
      System.out
          .println(transactionNum + "余额不足(" + account.getColdMoney() + "-" + unfreezeMoney + ")");
    }
    //重新计算冷热钱包的总额
    account.setColdMoney(account.getColdMoney().subtract(unfreezeMoney));
    account.setHotMoney(account.getHotMoney().add(unfreezeMoney));
  ...
  @Override
  public void saveMoney(BigDecimal money, String agentName, String website, String currencyType,
      String operator, String fixPriceCoinCode) {
  ...
  public void saveRecord(ExDigitalmoneyAccount eda, int type) {
    ...
    exDmTransaction.setTrueName(eda.getTrueName());
    exDmTransaction.setSurname(eda.getSurname());
    ...
    hotAccountRecord.setTrueName(eda.getTrueName());
