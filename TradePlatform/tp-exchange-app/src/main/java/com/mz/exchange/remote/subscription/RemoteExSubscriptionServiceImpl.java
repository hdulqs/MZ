package com.mz.exchange.remote.subscription;
...
import com.mz.exchange.subscription.service.ExSubscriptionPlanService;
...
public class RemoteExSubscriptionServiceImpl implements RemoteExSubscriptionService {
   ...
   * @throws:
   */
  public ExSubscriptionPlan subPlanInfo(ExSubscriptionPlan plan, Long customerId, String website,
      String currencyType) {
    //得到币账户信息
    ExDigitalmoneyAccount digaccount = exDigitalmoneyAccountService
        .getByCustomerIdAndType(customerId, plan.getCoinCode(), currencyType, website);
    //得到账户余额信息
    RemoteAppAccountService remoteappaccount = (RemoteAppAccountService) ContextUtil
        .getBean("remoteAppAccountService");
    AppAccount appaccount = remoteappaccount
    ...
    if (null != digaccount && null != appaccount) {
      ...
      //最多可以购买的金额 得到已经认购的价格
      List<ExSubscriptionRecord> totalMoney = exSubscriptionRecordService
          .subTotalNum(plan.getId(), customerId);
  ...
  public JsonResult remoteAddRecord(Long customerId, Long planId,
      String currentPrice, String buyTotalNum, String currencyType, String website,
    ...
    try {
      ...
      String newAccountPassWord = passwordHelper
          .encryString(accountPassword, appCustomer.getSalt());
      ...
      if (null != plan && null != digaccount && null != appaccount) {
        ...
        //得到已认购数量
        List<ExSubscriptionRecord> buyamount = exSubscriptionRecordService
            .subTotalNum(plan.getId(), customerId);
        //最多可以人认购金额
        ...
        jr = exSubscriptionRecordService
            .saveSubRecord(plan, appaccount, currentPrice, buyTotalNum, transactionNum);
        if (jr.getSuccess()) {
          //币账户增加余额
          BigDecimal number = new BigDecimal(buyTotalNum)
              .divide(new BigDecimal(currentPrice), 2, RoundingMode.FLOOR);
          exDigitalmoneyAccountService.inComeToHotAccount(digaccount.getId(), number,
              IdGenerate.transactionNum(NumConstant.App_Transaction), "认购", null, null);
          //账户余额减少
          remoteappaccount.payFromHotAccount(appaccount.getId(), new BigDecimal(buyTotalNum),
              IdGenerate.transactionNum(NumConstant.App_Transaction), "认购", null, null);
          //计算认购返佣金额
          ...
          remoteAppTradeFactorageService
              .dealCommissionBySubscription(plan, customerId, new BigDecimal(buyTotalNum),
                  transactionNum);
          //认购终止判断,
    ...
    if (null != backRecord) {
      if (backRecord.getCustomerId().longValue() == customreId.longValue()) {
        ...
        ExDigitalmoneyAccount account = exDigitalmoneyAccountService
            .getByCustomerIdAndType(backRecord.getCustomerId(), backRecord.getCoinCode(),
                ContextUtil.getCurrencyType(), ContextUtil.getWebsite());
        exDigitalmoneyAccountService
            .unfreezeAccountSelf(account.getId(), backRecord.getBackAmount(),
                IdGenerate.transactionNum(NumConstant.App_Transaction), "用户撤销回购", null, null);
  ...
  public ExSubscriptionPlan exPlanList(String state) {
    ...
    if (null != plan) {
      ...
      List<ExSubscriptionRecord> recordsize = exSubscriptionRecordService
          .sumCurrenNum(plan.getId());
      BigDecimal recordSum = recordsize.get(0).getCurrenNum();
      //认购数量、涨价基数大于0
      if (recordSum.compareTo(new BigDecimal(0)) > 0 && null != plan.getPriceBase()
          && plan.getPriceBase().compareTo(new BigDecimal("0")) > 0) {
        BigDecimal increaseMoney = recordSum.divide(plan.getPriceBase(), 0, RoundingMode.DOWN)
            .multiply(plan.getRose());
