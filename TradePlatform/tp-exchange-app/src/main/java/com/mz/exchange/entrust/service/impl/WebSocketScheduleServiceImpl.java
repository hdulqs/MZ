...
import com.mz.exchange.entrust.service.WebSocketScheduleService;
      ...
      if (!StringUtils.isEmpty(productListStr)) {
        ...
        for (String coinCodetwo : productList) {
          ...
          if (productCommon != null) {
            ...
            String header = entrustService
                .getHeader(Website, currencyType, coinCode, fixPriceCoinCode);
            //推送首页
            StringBuffer sb = new StringBuffer(
                "{\"coinCode\":\"" + productCommon.getCoinCode() + "\",\"fixPriceCoinCode\":\""
                    + fixPriceCoinCode + "\",\"name\":\"" + (null == productCommon.getName() ? ""
                    : productCommon.getName()) + coinCode + "\",\"sort\":\"" + (
                    null == productCommon.getSort() ? "1" : productCommon.getSort())
                    + "\",\"picturePath\":\"" + (null == productCommon.getPicturePath() ? ""
                    : productCommon.getPicturePath()) + "\",\"isRecommend\":\"" + productCommon
                    .getIsRecommend() + "\",\"data\":" + exOrderService.getTotal(header) + "}");
            PushData.pushIndex(sb.toString(), header);
          }
        }
      }
    ...
    for (String Website : mapLoadWeb.keySet()) {
      ...
      if (!StringUtils.isEmpty(productListStr)) {
        ...
        for (String coinCodetwo : productList) {
          ...
          String header = entrustService
              .getHeader(Website, currencyType, coinCode, fixPriceCoinCode);
          String ismatch = ExchangeDataCache
              .getStringData(header + ":" + ExchangeDataCache.IsMatch);
          if (StringUtil.isEmpty(ismatch) || ismatch.equals("1")) {
            String customerType = DifCustomer.getPushData(header);
            // 委托-----行情板块
            PushData.pushEntrusMarket(
                entrustService.getExEntrustChangeMarket(header, "0", 18, customerType),
                header);
            // 成交订单
            PushData
                .pushNewListRecordMarketDesc(exOrderService.getNewListMarket(header, 60, "desc"),
                    header);
      ...
      if (!StringUtils.isEmpty(productListStr)) {
        ...
        for (String coinCodetwo : productList) {
          ...
          if (!DifCustomer.getIsCommon()) {
            if (DifCustomer.getProductTransactionType(header).equals("2")) {
              String customerType1 = DifCustomer.getPushData1(header);
              PushData.pushtheSeatEntrustCenter(
                  entrustService.getExEntrustChangeMarket(header, "0", 18, customerType1), header);
              ...
              for (Integer j = 1; j <= 5; j++) {
                BigDecimal depth1 = depth.multiply(new BigDecimal(j));
                PushData.pushtheSeatEntrustDephCenter(
                    entrustService.getExEntrustChangeDephMarket(header, customerType1, 5, depth1),
                    header, j.toString());
