...
public class IcoCoinInterfaceUtil {
	...
	public static String create(String userName, String type) {
		...
		*/}else{
			...
			if (util.getIsOpen().equals("0")) {
				...
				LogFactory.info("开通币地址地址：" + url + "/coin/icoCoin/create?userName=" + userName + "&type=" + type);
				result = HttpsRequest.post(url + "/coin/icoCoin/create?userName=" + userName + "&type=" + type);
	...
	public static String lmcSendTo(String account, String address, String amount, String coinCode,String transactionNum,String userName) {
		...
		if (util.getIsOpen().equals("0")) {
			...
			String [] rs=remoteThirdInterfaceService.lmcTransfer(transfer);
			if(rs[0].equals(InterfaceState.SUCCESS)){
				ss="{code=8,msg="+rs[1]+"}";
			}
			LogFactory.info("提币接口调用结果："+ss);
		} else {
			ss = "{code=8,msg=接口没打开}";
		}
		return ss;
