 ...
 */
package com.mz.exchange.subscription.service.impl;
...
public class ExBuybackRecordServiceImpl extends BaseServiceImpl<ExBuybackRecord, Long> implements
    ExBuybackRecordService {
  ...
  public JsonResult saveBuybackRecord(ExSubscriptionRecord subRecord,
    ...
    //回购手续费金额
    BigDecimal feeMoney = subRecord.getRepurchaseFee()
  ...
  public JsonResult passedRecord(Long id) {
    ...
    if (exBuybackRecord != null && exBuybackRecord.getState() == 0) {
      ...
      ExDigitalmoneyAccount account = exDigitalmoneyAccountService
          .getByCustomerIdAndType(exBuybackRecord.getCustomerId(), exBuybackRecord.getCoinCode(),
      ...
      AppAccount appaccount = remoteappaccount
          .getByCustomerIdAndType(account.getCustomerId(), ContextUtil.getCurrencyType(),
              ContextUtil.getWebsite());
      //认购记录信息
      ExSubscriptionRecord subRecord = exSubscriptionRecordService
          .get(exBuybackRecord.getRecordId());
      if (null != account && null != appaccount && null != remoteappaccount && null != subRecord) {
        ...
        if (plan != null) {
          //计算回购金额
          BigDecimal incomeMoney = exBuybackRecord.getBackPrice()
              .multiply(exBuybackRecord.getBackAmount());
          //计算回购手续费金额
          //BigDecimal feeMoney = incomeMoney.multiply(plan.getRepurchaseFee().divide(new BigDecimal("100")));
          //增加账户余额
          remoteappaccount.inComeToHotAccount(appaccount.getId(),
              incomeMoney.subtract(exBuybackRecord.getFeeMoney()),
              IdGenerate.transactionNum(NumConstant.App_Transaction), "回购", null, null);
          //减少币种数量
          exDigitalmoneyAccountService
              .unfreezeAccountThem(account.getId(), exBuybackRecord.getBackAmount(),
                  IdGenerate.transactionNum(NumConstant.App_Transaction), "回购", null, null);
          //增加认购记录表的已回购数量
          ...
          //减少认购申请中的数量
          subRecord.setApplyBackNum(
              subRecord.getApplyBackNum().subtract(exBuybackRecord.getBackAmount()));
          exSubscriptionRecordService.update(subRecord);
          //更新回购记录表状态为1
          ...
          //更新认购计划表
          if (plan.getState() == 2) {
            plan.setPurchaseNumber(
                plan.getPurchaseNumber().subtract(exBuybackRecord.getBackAmount()));
    ...
    if (exBuybackRecord != null) {
      ...
      ExDigitalmoneyAccount account = exDigitalmoneyAccountService
          .getByCustomerIdAndType(exBuybackRecord.getCustomerId(), exBuybackRecord.getCoinCode(),
              ContextUtil.getCurrencyType(), ContextUtil.getWebsite());
      exDigitalmoneyAccountService
          .unfreezeAccountSelf(account.getId(), exBuybackRecord.getBackAmount(),
              IdGenerate.transactionNum(NumConstant.App_Transaction), "驳回回购", null, null);
      //减少回购申请中的数量
      ExSubscriptionRecord subRecord = exSubscriptionRecordService
          .get(exBuybackRecord.getRecordId());
      subRecord
          .setApplyBackNum(subRecord.getApplyBackNum().subtract(exBuybackRecord.getBackAmount()));
      exSubscriptionRecordService.update(subRecord);
      jr.setSuccess(true);
    } else {
      jr.setMsg("未查询到回购记录");
