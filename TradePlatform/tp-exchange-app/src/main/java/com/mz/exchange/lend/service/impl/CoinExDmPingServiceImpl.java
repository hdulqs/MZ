...
import com.mz.exchange.lend.PingLendDealTread;
import com.mz.exchange.lend.service.CoinExDmPingService;
...
public class CoinExDmPingServiceImpl extends BaseServiceImpl<ExDmPing, Long>
  ...
  public void endPing(String website, String currencyType) {
    ...
    for (ExDmPing l : listp) {
      ...
      Integer isPingWarehouse = isPingWarehouse(l.getCustomerId(), l.getUserCode(), currencyType,
          website);
  ...
  public void pingLend(String website, String currencyType) {
    ...
    for (ExDmLend e : list) {
      List<ExDmPing> l = getBycustomerid(e.getCustomerId(), e.getUserCode(), 1, currencyType,
          website);//已进入平仓流程的
      if (null == l || l.size() == 0) {
  ...
  @Override
  public void pingByCustomerId(Long customerId, String userCode, String currencyType,
      String website) {
  ...
  @Override
  public Integer isPingWarehouse(Long customerId, String userCode, String currencyType,
      String website) {
    BigDecimal netAsseToLend = accountService
        .netAsseToLend(customerId, null, currencyType, website);
    //杠杆的比例1.1，放文件
    ...
    BigDecimal lengRiskRate = new BigDecimal(
        StringUtil.isEmpty(lengRiskRates) ? "0" : lengRiskRates); //120
  ...
  public void pingCoinFlow(Long customerId, String userCode) {
    ...
    exDmPing
        .setPingNum(IdGenerate.transactionNum(IdGenerate.transactionNum(NumConstant.Ex_Dm_Ping)));
    ...
    for (ExDmLend l : list) {
      ExDigitalmoneyAccount eda = exDigitalmoneyAccountService
          .getByCustomerIdAndType(customerId, null, l.getCurrencyType(), l.getWebsite());
      //	eda.getHotMoney().add()
    }
...
//			 examineOrderService.stopeAlllistByapply(customerId, currencyType, website);
      ...
      List<ExDigitalmoneyAccount> list = exDigitalmoneyAccountService
          .getByCustomerId(customerId, currencyType, website);
      System.out.println(
          "ExDigitalmoneyAccount l:list==" + customerId + "=" + currencyType + "=" + website);
      System.out.println("ExDigitalmoneyAccount l:list==" + list.size());
      for (ExDigitalmoneyAccount l : list) {
        System.out.println("平l.getHotMoney()=" + l.getCoinCode() + "=" + l.getHotMoney());
        addExEntrust(2, l.getCustomerId(), userCode, l.getCoinCode(), l.getHotMoney(), currencyType,
            website);
      }
      //3自动还钱（等币都卖完了就自动还钱，市价正常委托出去就里面会匹配所以稍微等会就可以还钱）
      PingLendDealTread pingLendDealTread = new PingLendDealTread(customerId);
      com.mz.core.thread.ThreadPool.exe(pingLendDealTread);
    }


  }

  public void addExEntrust(Integer type, Long customerId, String userCode, String coinCode,
  ...
  @Override
  public List<ExDmPing> getBycustomerid(Long customerId, String userCode, Integer status,
