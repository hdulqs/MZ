...
public class AppPersonInfoServiceImpl extends BaseServiceImpl<AppPersonInfo, Long> implements AppPersonInfoService{
	...
	public AppPersonInfo getByCustomerId(Long customerId){
		...
		if(null!=customerId && !"".equals(customerId)){
			...
			List<AppPersonInfo> list=((AppPersonInfoDao)dao).getByCustomerId(map);
	...
	public PageResult findPageBySql1(QueryFilter filter) {
		...
		//----------------------分页查询底部外壳------------------------------
		//处理分页数据
		List<AppPersonInfo> list=page.getResult();
		for (int i = 0; i < list.size(); i++) {
			AppPersonInfo personinfo=list.get(i);
			//获取总充值
			personinfo.setTotalRechargeMoney(((AppPersonInfoDao)dao).getTotalRechargeMoney(personinfo.getCustomerId()));
			//获取总充值
			personinfo.setTotalWithdrawalsMoney(((AppPersonInfoDao)dao).getTotalWithdrawalsMoney(personinfo.getCustomerId()));
			//资金变动值(=总资产-总充值），
			personinfo.setMoneyChangeRate(personinfo.getTotalAvailableMoney().add(personinfo.getTotalFrozenMoney()).subtract(personinfo.getTotalWithdrawalsMoney()));
			//收益率 收益率（=总资产-总充值）/总充值
			BigDecimal bbb=BigDecimal.ZERO;
			if(personinfo.getTotalRechargeMoney().compareTo(BigDecimal.ZERO)!=0){//当总充值不为0
				bbb=personinfo.getMoneyChangeRate().divide(personinfo.getTotalRechargeMoney(),2,BigDecimal.ROUND_HALF_DOWN);
			}
			personinfo.setProfitRate(bbb);
