...
public class AppTradeFactorageServiceImpl extends
    BaseServiceImpl<AppTradeFactorage, Long> implements AppTradeFactorageService {
    ...
    if (null != list && list.size() > 0) {
      for (int i = 0; i < list.size(); i++) {
        ...
        if (transactionBuyFee.compareTo(BigDecimal.ZERO) > 0) {
          ...
          List<AppAgentsCustromer> list2 = this
              .saveAppTradeFactorage1(buyCustomer, transactionBuyFee, fixPriceCoinCode,
                  fixPriceType, type);
          if (list2.size() > 0) {
            for (int j = 0; j < list2.size(); j++) {
              if (list2.get(j) != null) {
                commissionDetailService.saveCommissionDetailForOrder(orderInfo, list2.get(j),
                    Integer.valueOf(list2.get(j).getFrontBank()));
              }
            }
          }
          return true;
  ...
  public Boolean dealCommissionByTraction(String tractionNum, Integer type) {
    ...
    AppTransaction appTransaction = remoteAppTransactionService
        .createTranctonByOrderNum(tractionNum);
  ...
  public Boolean dealCommissionBySubscription(ExSubscriptionPlan plan, Long customerId,
    ...
    AppAgentsCustromer agentsCustromerOne = appAgentsCustromerService
        .findAgentsByCustromer(customer.getUserName(), 1);
    if (null != agentsCustromerOne && null != plan.getSratioOne()) {
      ...
      if (sration.compareTo(new BigDecimal("0")) > 0) {
        BigDecimal srationMoney = buyTotalNum
            .multiply(sration.divide(new BigDecimal("100"), 4, RoundingMode.DOWN));
        //每个用户给他的父交的钱
        ...
        commissionDetailService
            .saveCommissionDetailSubscription(customer, agentsCustromerOne, srationMoney,
                transactionNum, 1, sration);
      }
    }
    //查询是否有二级推荐人信息，是否设置了二级推荐奖励
    AppAgentsCustromer agentsCustromerTwo = appAgentsCustromerService
        .findAgentsByCustromer(customer.getUserName(), 2);
    if (null != agentsCustromerTwo && null != plan.getSratioTwo()) {
      ...
      if (sration.compareTo(new BigDecimal("0")) > 0) {
        ...
        //佣金明细
        commissionDetailService
            .saveCommissionDetailSubscription(customer, agentsCustromerTwo, srationMoney,
                transactionNum, 2, sration);
      }
    }
    //查询是否有三级推荐人信息，是否设置了三级推荐奖励
    AppAgentsCustromer agentsCustromerTheree = appAgentsCustromerService
        .findAgentsByCustromer(customer.getUserName(), 3);
    if (null != agentsCustromerTheree && null != plan.getSratioTheree()) {
      ...
      if (sration.compareTo(new BigDecimal("0")) > 0) {
        ...
        //佣金明细
        commissionDetailService
            .saveCommissionDetailSubscription(customer, agentsCustromerTheree, srationMoney,
   ...
   * @throws:
   */
  public void saveSubscriptionAppTradeFactorage(AppAgentsCustromer customer,
      BigDecimal srationMoney, Integer sration) {
   ...
   *
   * customer 用户 commissionMoney 计算佣金的钱数 type  2 表示交易  1 表示提现
   */
  public List<AppAgentsCustromer> saveAppTradeFactorage1(AppCustomer customer,
      BigDecimal commissionMoney, String fixPriceCoinCode, int fixPriceType, int type) {

    AppTradeFactorage factorage = null;

    List<AppAgentsCustromer> list2 = new ArrayList<AppAgentsCustromer>();

    List<AppAgentsCustromer> list = commissionDeployService
        .findByTransaction(customer.getUserName());
    System.out.println("传入username=" + customer.getUserName());

    if (list.size() > 0) {
      System.out.println("有父节点会进来");
      AppTradeFactorage factorage1 = this
          .findFactorageByCustromerId1(customer.getId(), fixPriceCoinCode);
      ...
      if (null != commissionDeploy) {
        ...
        BigDecimal bigDecimalOneRate = commissionDeploy.getOneRankRatio()
        ...
        // 判断用户的手续费是否大于后台配置的佣金标准
        if (list.size() > 1
            && commissionDeploy.getTwoStandardValue().compareTo(commissionMoney) <= 0) {
          BigDecimal bigDecimalTwoRate = commissionDeploy.getTwoRankRatio()
              .divide(new BigDecimal("100"));
        ...
        // 判断用户的手续费是否大于后台配置的佣金标准
        if (list.size() > 2
            && commissionDeploy.getThreeStandardValue().compareTo(commissionMoney) <= 0) {
          BigDecimal bigDecimalTwoRate = commissionDeploy.getThreeRankRatio()
              .divide(new BigDecimal("100"));
   ...
   * customer 用户 commissionMoney 计算佣金的钱数 type  2 表示交易  1 表示提现
   */
  public List<AppAgentsCustromer> saveAppTradeFactorage(AppCustomer customer,
      BigDecimal commissionMoney, int type) {
