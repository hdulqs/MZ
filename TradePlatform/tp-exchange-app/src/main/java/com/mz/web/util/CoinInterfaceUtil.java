...
public class CoinInterfaceUtil {
	...
	public static String  create(String userName,String type) {
		...
		if(util.getIsOpen().equals("0")){
			...
			LogFactory.info("开通币地址地址："+url+"/coin/coin/create?userName="+userName+"&type="+type);
			ss=HttpsRequest.post(url+"/coin/coin/create?userName="+userName+"&type="+type);
			LogFactory.info("钱包调用返回值="+ss);
		}else{
	...
	public static String  list(String type) {
		...
		if(util.getIsOpen().equals("0")){
			String url=PropertiesUtils.APP.getProperty("app.coinip");
			ss=HttpsRequest.post(url+"/coin/coin/allList?type="+type);
	...
	public static String  listForUser(String type,String userName) {
		...
		if(util.getIsOpen().equals("0")){
			String url=PropertiesUtils.APP.getProperty("app.coinip");
			ss=HttpsRequest.post(url+"/coin/coin/list?type="+type+"&userName="+userName);
		}else{
			ss="[]";
		}
		return ss;
	...
	public static String  sendTo(String account,String address,String amount,String coinCode) {
		...
		if(util.getIsOpen().equals("0")){
			String url=PropertiesUtils.APP.getProperty("app.coinip");
			ss=HttpsRequest.post(url+"/coin/coin/sendFrom?type="+coinCode+"&"+"account="+account+"&"+"address="+address+"&"+"amount="+amount);
		}else{
			ss="{code=8,msg=接口没打开}";
		}
	...
	public static String  row(String orderNo,String type) {
		...
		if(util.getIsOpen().equals("0")){
			String url=PropertiesUtils.APP.getProperty("app.coinip");
			ss=HttpsRequest.post(url+"/coin/coin/row?orderNo="+orderNo+"&type="+type);
	...
	public static String  sendTo(String address ,String amount,String type) {
		...
		if(util.getIsOpen().equals("0")){
			String url=PropertiesUtils.APP.getProperty("app.coinip");
			ss=HttpsRequest.post(url+"/coin/coin/sendTo?address="+address+"&type="+type+"&amount="+amount);
    	}else{
	...
	public static String  sendFrom(String account, String address ,String amount,String type) {
		...
		if(util.getIsOpen().equals("0")){
			String url=PropertiesUtils.APP.getProperty("app.coinip");
			ss=HttpsRequest.post(url+"/coin/coin/sendFrom?account="+account+"&address="+address+"&type="+type+"&amount="+amount);
		}else{
	...
	public static String  listAccounts(String type) {
		...
		if(util.getIsOpen().equals("0")){
			String url=PropertiesUtils.APP.getProperty("app.coinip");
			ss=HttpsRequest.post(url+"/coin/coin/listAccounts?type="+type);
		}else{
			ss="{code=18,msg=222222}";                                          
		}
		return ss;
	...
	public static Map<String, Object>  getAllUsers(String type) {
		...
		if(util.getIsOpen().equals("0")){
			String url=PropertiesUtils.APP.getProperty("app.coinip");
			ss=HttpsRequest.post(url+"/coin/coin/getAllUsers?type="+type);
			if("" != ss){
	 ...
	 public   String getIsOpen(){
		...
		public static String  listByCustomerName(String type,String customerName) {
			...
			if(util.getIsOpen().equals("0")){
				...
				ss=HttpsRequest.post(url+"/coin/coin/list?type="+type+"&userName="+customerName);
				
				
				//System.out.println("====222266");
