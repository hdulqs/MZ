 ...
 */
@Controller
@RequestMapping("/app/applendconfig")
public class AppLendConfigController extends BaseController<AppConfig, Long> {

  @Autowired
  private RedisService redisService;


  @Resource(name = "appLendConfigService")
  @Override
    ...
    for (int i = 0; i < split.length; i++) {
      if ("configType".equals(split[i].split("!!=")[0])) {
        if (split[i].split("!!=")[1] != null) {
                    ...
                    if (!key.equals(con.getValue())) {
                      ...
                      if (confgiKey.equals("openAndclosePlateTime")) {
                        ...
                        if (!before[after.length - 1].equals(after[after.length - 1])) {
                          RemoteAppReportSettlementService remoteAppReportSettlementService = (RemoteAppReportSettlementService) ContextUtil
                              .getBean("remoteAppReportSettlementService");
          ...
          List<AppConfig> appConfigList = ((AppLendConfigService) service).getConfig(map);
  ...
  public List<AppConfig> list(HttpServletRequest request) {
    ...
    filter.addFilter("typekey=", "financeLendConfig");
    filter.or("typekey=", "financeProtocol");
  ...
  @MethodName(name = "查询appLendConfig中的configkey")
  @RequestMapping("/find")
  @ResponseBody
  @MyRequiresPermissions
  public List<AppConfig> find(HttpServletRequest request) {
    return ((AppLendConfigService) service).findKey();
  }
