...
public class AppOurAccountServiceImpl extends BaseServiceImpl<AppOurAccount, Long> implements
    AppOurAccountService {
   ...
   */
  @Override
  public boolean sendCoinToAgent(ExOrderInfo orderInfo, Long agentsCustromerId, int needGiveNum,
      String coinCode) {
    boolean res = false;
    //推荐人
    ...
    try {
      ...
      System.out.println("给推荐人币账户增加对应的币,并保存派发记录异常！！！");
  ...
  public void sendAuthRewardToCustomer(Long customerId, String coinCode) {
    ...
    try {
      ...
      System.out.println("给币账户增加实名奖励的币,并保存派发记录异常！！！");
      System.out.println(e.getMessage());
    }
   ...
   * @throws:
   */
  public void sendTradefeeToAgent(AppPersonInfo personInfo, BigDecimal money,
      ExOrderInfo orderInfo) {
    boolean res = false;
    AppAgencyTranAwardRecordService awardRecordService = (AppAgencyTranAwardRecordService) ContextUtil
        .getBean("appAgencyTranAwardRecordService");

    try {
      //先获取资金账户
      AppAccountService accountService = (AppAccountService) ContextUtil
      ...
      System.out.println("卖币人给推荐人或者省市县返佣异常！！！");
      System.out.println(e.getMessage());
    }
    ...
    awardRecord
        .setSellPersonId(appPersonInfoService.getByCustomerId(orderInfo.getSellCustomId()).getId());
    if (res) {
  ...
  public void judgeAuthRewardState(String userName, Long exdmaccountId) {
    System.out.println("金科比地址生成成功后，给实名人返还一个币:" + userName);
    ...
    System.out.println("配置的实名阈值:" + authRewardNum + ",所有实名的人数量:" + hasAuthNum);
