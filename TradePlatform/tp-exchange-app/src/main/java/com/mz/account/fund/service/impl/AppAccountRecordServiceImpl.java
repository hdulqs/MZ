...
//	Map<Integer, Integer> map = new HashMap<Integer, Integer>();
	 ...
	 */
	@Override
	public void addOurRecord(BigDecimal money,AppOurAccount appOurAccount,AppAccount appAccount,
			ExOrderInfo orderInfo,Map<String,Integer> map, String auditor,String remark){
	...
	@Override
	public void ourAccountChangeRecord(AppOurAccount ourAccount, String exDigNum, AppIcoCoinTransaction coinTransaction,
			String auditor, String remark) {
		if (null != ourAccount && null != coinTransaction) {
			AppAccountRecord appAccountRecord = new AppAccountRecord();
			appAccountRecord.setAppAccountNum(ourAccount.getAccountNumber());
			appAccountRecord.setAppAccountId(ourAccount.getId());
			appAccountRecord.setCustomerId(coinTransaction.getCustomerId());
			appAccountRecord.setCustomerName(coinTransaction.getCustomerName());
			Integer type = Integer.valueOf(ourAccount.getOpenAccountType());
			if (type == 0) {
				appAccountRecord.setRecordType(1);
			} else if (type == 1) {
				appAccountRecord.setRecordType(2);
			} else if (type==3){//ICO提币
				appAccountRecord.setRecordType(10);
			}else {
				appAccountRecord.setRecordType(type);
			}

			appAccountRecord.setSource(1);
			appAccountRecord.setTransactionMoney(coinTransaction.getTransactionMoney());
			appAccountRecord.setTransactionNum(coinTransaction.getTransactionNum());
			appAccountRecord.setStatus(5);
			appAccountRecord.setRemark(remark);
			appAccountRecord.setCurrencyName(coinTransaction.getCoinCode());
			appAccountRecord.setCurrencyType(null);
			appAccountRecord.setAuditor(auditor);
			appAccountRecord.setOperationTime(new Date());
			appAccountRecord.setCustomerAccount(exDigNum);
