...
public class AppReportSettlementCulImpl extends
  ...
  public Map<String, BigDecimal> getReWiByAppTransaction(Long customerId, String currencyType,
    ...
    // 查出所以充值提现的成功数据
    List<AppTransaction> rechargeList = remoteAppTransactionService
...
/*		Map<String, Object> map1 = new HashMap<String, Object>();
  ...
  public Map<String, Object> culAccountByCustomer(Long customerId, String currencyType,
    ...
    for (ExDigitalmoneyAccount eda : listeda) {
      ...
      BigDecimal edhotMoney = coinsureoldHotCount.subtract(coinsureoldLendCount).add(inCoinCount)
          .subtract(inCoinFee).subtract(outCoinCount)
          ...
          .subtract(transactionFeeFixPrice).subtract(transactionFeecoin)
          .subtract(coldEntrustFixPrice).add(coindrawalMoney);
      ...
      BigDecimal otchotcount = getotccount.get("otcoldcount");
  ...
  public ExDigitalmoneyAccountRedis getExDigitalmoneyAccountByRedis(String id) {
    ...
    if (null == obj) {
      ...
      ExDigitalmoneyAccountRedis accountr = (ExDigitalmoneyAccountRedis) JSON
          .parseObject(JSON.toJSONString(account), ExDigitalmoneyAccountRedis.class);
      return accountr;
  ...
  public AppAccountRedis getAppAccountByRedis(String id) {
    ...
    if (null == obj) {
      ...
      AppAccountRedis accountr = (AppAccountRedis) JSON
          .parseObject(JSON.toJSONString(account), AppAccountRedis.class);
      return accountr;
    } else {
  ...
  public String[] settlementByCustomerId(AppCustomer l, Date comeDate, Date nowDate, Date endDate,
    ...
    // 查出所以充值提现的成功数据,充值额,提现额，提现手续费
    Map<String, BigDecimal> getReWiByAppTransaction = getReWiByAppTransaction(customerId,
        currencyType, website, beginDateString, endDateString);
    ...
    Map<String, BigDecimal> getTransactionByExEntrust = getTransactionByExEntrust(customerId,
        currencyType, website, beginDateString, endDateString);
    exSettlementFinance
        .setBuyTransactionMoney(getTransactionByExEntrust.get("buyTransactionMoney"));
    exSettlementFinance
        .setSellTransactionMoney(getTransactionByExEntrust.get("sellTransactionMoney"));
    ...
    AppAccount appAccount = remoteAppAccountService
        .getByCustomerIdAndType(l.getId(), currencyType, website);
    if (null == appAccount) {
      System.out.println(l.getId() + currencyType + website + "资金账号为空");
      ...
      // 期末净资产
      BigDecimal rMBNetAsset = remoteExDigitalmoneyAccountService
          .getRMBNetAsset(l.getId(), null, currencyType, website);
    ...
    if (null == fiarsclist || fiarsclist.size() == 0) {
      ...
      exSettlementFinance
          .setBeginMoney(fiarsclist.get(fiarsclist.size() - 1).getEndMoney()); // 期初资产

      exSettlementFinance
          .setBeginNetAsset(fiarsclist.get(fiarsclist.size() - 1).getEndNetAsset());// 期末净资产
    }
    // 融资
    Map<String, BigDecimal> getLendByExDmLend = getLendByExDmLend(customerId, currencyType,
        currencyType, website, beginDateString, endDateString);
    ...
    profitAndLossMoney = exSettlementFinance.getEndNetAsset()
        .subtract(exSettlementFinance.getBeginNetAsset());
    profitAndLossMoney = profitAndLossMoney.subtract(exSettlementFinance.getRechargeMoney())
        .add(exSettlementFinance.getWithdrawMoney());
    ...
    for (ExDigitalmoneyAccount eda : listeda) {
      ...
      // 计算币的买成交量，卖成交量
      Map<String, BigDecimal> getTranCoinByExEntrust = getTranCoinByExEntrust(customerId, coinCode,
          currencyType, website, beginDateString, endDateString);
      ...
      String currentExchangPrices = ExchangeDataCache.getStringData(
          eda.getWebsite() + ":" + eda.getCurrencyType() + ":" + eda.getCoinCode() + ":"
      ...
      // 币的转入转出量
      Map<String, BigDecimal> getInOutByExDmTransaction = getInOutByExDmTransaction(customerId,
          coinCode, currencyType, website, beginDateString, endDateString);
      ...
      // 融币
      Map<String, BigDecimal> getLendcoinByExDmLend = getLendcoinByExDmLend(customerId, coinCode,
          currencyType, website, beginDateString, endDateString);
      coin.setLendMoney(getLendcoinByExDmLend.get("lendCoin"));

      // 已还融币
      Map<String, BigDecimal> getRepaycoinExDmLendIntent = getRepaycoinExDmLendIntent(customerId,
          coinCode, currencyType, website, beginDateString, endDateString);
      coin.setRepaylendMoney(getRepaycoinExDmLendIntent.get("repaylendcoin"));

      profitAndLossMoney = profitAndLossMoney.add(
          (coin.getOutCoinCount().subtract(coin.getInCoinCount())).multiply(currentExchangPrice));
  ...
  public Map<String, BigDecimal> getCoinByAccountDisable(Long customerId, String coinCode,
    ...
    List<AppAccountDisable> appAccountDisablelist = remoteExDigitalmoneyAccountService
        .findAppAccountDisable(transactionMoneyfilter1);
