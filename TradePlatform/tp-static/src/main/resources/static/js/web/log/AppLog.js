 ...
 * AppUser.js
 */
define(['app','hryTable','bootstrap-datetimepicker', 'hryUtil','pikadayJq' ], function (app, DT) {
    ...
    function controller($scope,$rootScope,$http,$stateParams,hryCore ){
        ...
        if($stateParams.page=="list"){
        	...
        	var config = DT.config();
        		...
        		config.ajax.url = HRY.modules.web+'log/applog/list';
        		config.ajax.data = function(d){
    				...
    				DT.inputData($scope.serchData);
						        			$.each($scope.serchData, function(name,value){
												if(""!=value){
													debugger;
													if("systemTime_like"==name){
														//需要把格式为2017-5-1转成2017-05-01
