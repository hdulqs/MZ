...
define(['app','hryTable'], function (app, DT) {
	...
	//------------------------查看页面路径---------------------------------------------
		if ($stateParams.page == "seeinfo") {
    
			 hryCore.CURD({
						url : HRY.modules.web+"app/appapi/seeinfo/"+$stateParams.id
					}).get(null,
				function(data) {
            		$scope.formData = data;
			    }, function(data) {
...
//------------------------增加页面路径------------------------------------------------------------------------------------------------------
		...
		if($stateParams.page=="add"){
			...
			$scope.processForm = function() {
						...
						$http({
		 					method : 'POST',
		 					url : HRY.modules.web+'app/appapi/add',
 ...
 //------------------------修改页面路径-----------------------------------------------------------------------------------------------     
        ...
        if($stateParams.page=="modify"){
				...
				$http.get(HRY.modules.web+"app/appapi/see/"+$stateParams.id).
                success(function(data) {
                	 $scope.model = data;
                	 $scope.formData = data;
				...
				$scope.processForm = function() {
					delete $scope.formData['list']
					
				  $http({
						method : 'POST',
						url : HRY.modules.web+'/app/appapi/modify',
						data : $.param($scope.formData), 
						headers : {
							'Content-Type' : 'application/x-www-form-urlencoded'
						}
 ...
 //------------------------列表页面路径-------------------------------------------------------------------------------------------------------
        ...
        if($stateParams.page=="list"){
        	...
        	var config = DT.config();
        		...
        		config.ajax.url = HRY.modules.web+'/app/appapi/list';
            ...
            function fnSee(url){
            	...
            	if(ids.length==0){
            		...
            		window.location.href='#/web/'+url+'/'+ids;
