...
define(['app','hryTable'], function (app) {
    ...
    function controller($scope,$rootScope,$http,$stateParams){
   	 ...
   	 $http.get( HRY.modules.web+'app/appconfig/find').
     success(function(data) {
        ...
        if($stateParams.page=="add"){
         $scope.processForm = function() {
        	 $scope.formData.typekey= $("#appRoleSet").val();
        	    $http({
					method : 'POST',
					url : HRY.modules.web+'app/appconfig/add.do',
        ...
        if($stateParams.page=="list"){
            ...
            $scope.conType='baseConfig';//默认显示基础配置
            $('input[name=typeConfig]').val("baseConfig");
             ...
             */
            function fnList(){
            	 $http.get( HRY.modules.web+'app/appconfig/list').
                 success(function(data) {
                	$scope.list = data;
                 });
            }
        	...
        	* @throws
            ...
            data = data.replace(/\+/g,"^");
         	    $http({
    				method : 'POST',
    				url : HRY.modules.web+'app/appconfig/modify',
    				data : data, 
    				headers : {
    					'Content-Type' : 'application/x-www-form-urlencoded'
    				}
