...
define(['app','hryTable'], function (app, DT) {
    ...
    function controller($scope,$rootScope,$http,$stateParams,hryCore ){
		...
		if ($stateParams.page == "see") {
			 hryCore.CURD({
						url : HRY.modules.web+"article/appcategory/see/"+$stateParams.id
					}).get(null,
				function(data) {
             	$scope.model = data;
             	//渲染权限树
...
//---------------------------------------------------------------------
		...
		if($stateParams.page=="add"){
			...
			$scope.processForm = function() {
				...
				$scope.formData.preateId = $("#preateId").val();
				$scope.formData.type = $("#type").val();
					$http({
						method : 'POST',
						url : HRY.modules.web+'article/appcategory/add',
					...
					.success(function(data) {
								if (data.success) {
									...
									window.location.href='#/web/article/appcategory/list/'+website;
								} else {
      ...
      //------------------------修改页面路径---------------------------------------------
        if($stateParams.page=="modify"){
        	$http.get(HRY.modules.web+"article/appcategory/see/"+$stateParams.id).
                 success(function(data) {
                	//  
                 	 $scope.model = data;
                 	 $scope.formData = data;
      ...
      //  	$scope.formData = {};
			$scope.processForm = function() {
					...
					$http({
						method : 'POST',
						url : HRY.modules.web+'article/appcategory/modify',
						data : $.param($scope.formData), 
						headers : {
							'Content-Type' : 'application/x-www-form-urlencoded'
						}
					...
					.success(function(data) {
								...
								if (!data.success) {
									...
									window.location.href='#/web/article/appcategory/list/'+$scope.formData.website;
								}
						});
	 		  	 });
			};
        ...
        if($stateParams.page=="list"){
        	...
        	var config = DT.config();
        		...
        		config.ajax.url = HRY.modules.web+'article/appcategory/list';
