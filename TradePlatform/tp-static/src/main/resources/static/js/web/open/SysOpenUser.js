...
define([ 'app', 'hryTable', 'layer' ], function(app, DT, layer) {
	...
	function controller($scope, $rootScope, $http, $stateParams, $state, hryCore) {
		...
		if ($stateParams.page == "see") {
			$http.get(HRY.modules.web + "open/sysopenuser/see/" + $stateParams.id).success(function(data) {
				$scope.formData = data;
			});
		}
		...
		if ($stateParams.page == "add") {
			...
			$scope.processForm = function() {
				$http({
					method : 'POST',
					url : HRY.modules.web + 'open/sysopenuser/add',
					...
					if (data.success) {
						...
						window.location.href = '#/web/open/sysopenuser/list/anon';
					} else {
		...
		if ($stateParams.page == "modify") {
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.web + 'open/sysopenuser/modify',
					data : $.param($scope.formData),
					headers : {
						'Content-Type' : 'application/x-www-form-urlencoded'
					}
		...
		if ($stateParams.page == "list") {
			...
			config.ajax.url = HRY.modules.web + 'open/sysopenuser/list';
			...
			$scope.fnSee = function(){
				...
				if (ids.length == 0) {
					...
					window.location.href = "#/web/open/sysopenuser/see/" + ids[0];
				}
			}
			...
			$scope.fnAdd = function(){
				window.location.href = "#/web/open/sysopenuser/add/anon";
			}
			...
			$scope.fnModify = function(){
            	...
            	if (ids.length == 0) {
        			...
        			window.location.href='#/web/open/sysopenuser/modify/'+ids[0];
            	}
			}
			...
			$scope.fnRemove = function() {
				...
				layer.confirm("你确定删除？", {
					...
					hryCore.CURD({
						url : HRY.modules.web + "open/sysopenuser/remove/"+ ids
