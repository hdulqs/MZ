...
define(['app','hryTable'], function (app, DT) {
    ...
    function controller($scope,$rootScope,$http,$stateParams,hryCore ){
    	$rootScope.headTitle = $rootScope.title = "account管理";
    	//初始化js 插件
		...
		if ($stateParams.page == "see") {
			 hryCore.CURD({
						url : HRY.modules.oauth+"account/appfundaccount/see/"+$stateParams.id
					}).get(null,
				function(data) {
             	$scope.model = data;
             	//渲染权限树
        ...
        if($stateParams.page=="add"){
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.exchange+'account/appfundaccount/add',
				...
				.success(function(data) {
							if (data.success) {
								...
								window.location.href='#/exchange/account/appfundaccount/list/anon';
							} else {
      ...
      //------------------------修改页面路径---------------------------------------------
        if($stateParams.page=="modify"){
        	$http.get(HRY.modules.exchange+"account/appfundaccount/see/"+$stateParams.id).
                 success(function(data) {
                 	 $scope.model = data;
                 	 $scope.formData = data;
                	...
                	function loadMenuList(){
                		$.post(HRY.modules.exchange+"user/approle/findMyResourceAndNohasResource?id="+$stateParams.id,
                			 null,
                	  		 function(data){
      ...
      //  	$scope.formData = {};
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.exchange+'account/appfundaccount/modify',
					data : $.param($scope.formData), 
					headers : {
						'Content-Type' : 'application/x-www-form-urlencoded'
					}
        ...
        if($stateParams.page=="list"){
        	...
        	var config = DT.config();
        		...
        		config.ajax.url = HRY.modules.exchange+'account/exapiapply/list';
            ...
            function fnModify(url){
            	...
            	if(ids.length==0){
            		...
            		$rootScope.id= ids[0];
            		window.location.href='#/oauth/'+url+'/'+b[0];
            	}
            }
            ...
            function fnRemove(url){
            	...
            	hryCore
				.CURD({
						url:HRY.modules.exchange+url+"?id="+id
				 })
				.remove(null,
						function(data) {
		                	if(data.success){
