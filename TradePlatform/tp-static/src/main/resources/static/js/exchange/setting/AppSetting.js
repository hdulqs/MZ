...
define(['app','hryTable'], function (app, DT) {
    ...
    function controller($scope,$rootScope,$http,$stateParams,hryCore ){
		...
		if ($stateParams.page == "see") {
			 hryCore.CURD({
						url : HRY.modules.web+"setting/appsetting/see/"+$stateParams.id
					}).get(null,
				function(data) {
             	$scope.model = data;
           });
        ...
        if($stateParams.page=="add"){
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.web+'setting/appsetting/add',
				...
				.success(function(data) {
							if (data.success) {
								...
								window.location.href='#/web/setting/appsetting/list/anon';
							} else {
      ...
      //------------------------修改页面路径---------------------------------------------
        if($stateParams.page=="modify"){
        	$http.get(HRY.modules.web+"setting/appsetting/see/"+$stateParams.id).
                 success(function(data) {
                 	 $scope.model = data;
                 	 $scope.formData = data;
             });
			$scope.processForm = function() {
				$http({
					method : 'POST',
					url : HRY.modules.web+'setting/appsetting/modify',
					data : $.param($scope.formData), 
					headers : {
						'Content-Type' : 'application/x-www-form-urlencoded'
					}
        ...
        if($stateParams.page=="list"){
        	...
        	var config = DT.config();
        		...
        		config.ajax.url = HRY.modules.web+'setting/appsetting/list';
