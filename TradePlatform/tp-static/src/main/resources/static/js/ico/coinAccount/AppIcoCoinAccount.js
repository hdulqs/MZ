...
define([ 'app', 'hryTable'  ,'hryUtil' ], function(app, DT) {
	...
	function controller($scope, $rootScope, $http, $stateParams, hryCore, $location) {
		...
		if ($stateParams.page == "see") {
			...
			$http.get(HRY.modules.exchange + "coinAccount/appicocoinaccount/see/" + $stateParams.id).success(function(data) {
		...
		if ($stateParams.page == "list") {
			...
			config.ajax.url = HRY.modules.exchange + 'coinAccount/appicocoinaccount/list';
			...
			//禁用
			var ermissions=$rootScope.requiresPermissions('exchange','/coinAccount/appicocoinaccount/disable');
			config.columns = [ {
				"data" : "id"
			}, {
				"data" : "userName"
			...
			function refreshUserCoin(url) {
				...
				if (ids.length == 0) {
					...
					var parameter={coinCode:obj.coinCode,account:obj.userName,count:50};
					hryCore.CURD({
						url : HRY.modules.exchange + "/coinAccount/appicocoinaccount/refreshUserCoin" ,
						parameter : parameter
				...
				if (ids.length == 0) {
					...
					if(data[0].hotMoney>0){
						...
						$scope.fnExDisableSubmit = function(){
							$http({
								method : 'POST',
								url : HRY.modules.exchange + '/coinAccount/appicocoinaccount/disable',
			...
			$scope.fnCoinPoking = function(){
				...
				if (ids.length == 0) {
					...
					$scope.fnCoinPokingSubmit = function(){
						$http({
							method : 'POST',
							url : HRY.modules.exchange + '/coinAccount/appicocoinaccount/coinPoking',
			...
			$scope.fnRecharge = function(){
				...
				if (ids.length == 0) {
					...
					$scope.fnRechargeSubmit = function(){
						$http({
							method : 'POST',
							url : HRY.modules.exchange + '/coinAccount/appicocoinaccount/recharge',
