...
define([ 'app', 'hryTable', 'layer' ], function(app, DT, layer) {
	...
	function controller($scope, $rootScope, $http, $stateParams, $state, hryCore) {
		...
		if ($stateParams.page == "see") {
			$http.get(HRY.modules.ico + "coin/appicocoin/see/" + $stateParams.id).success(function(data) {
				$scope.formData = data;
			});
		}
		...
		if ($stateParams.page == "add") {
			...
			$scope.processForm = function() {
				$http({
					method : 'POST',
					url : HRY.modules.ico + 'coin/appicocoin/add',
					...
					if (data.success) {
						...
						window.location.href = '#/ico/coin/appicocoin/list/anon';
					} else {
		...
		if ($stateParams.page == "modify") {
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.ico + 'coin/appicocoin/modify',
					data : $.param($scope.formData),
					headers : {
						'Content-Type' : 'application/x-www-form-urlencoded'
					}
		...
		if ($stateParams.page == "list") {
			...
			config.ajax.url = HRY.modules.ico + 'coin/appicocoin/list';
			...
			$scope.fnSee = function(){
				...
				if (ids.length == 0) {
					...
					window.location.href = "#/ico/coin/appicocoin/see/" + ids[0];
				}
			}
			...
			$scope.fnAdd = function(){
				window.location.href = "#/ico/coin/appicocoin/add/anon";
			}
			...
			$scope.fnModify = function(){
            	...
            	if (ids.length == 0) {
        			...
        			window.location.href='#/ico/coin/appicocoin/modify/'+ids[0];
            	}
			}
			...
			$scope.fnRemove = function() {
				...
				layer.confirm("你确定删除？", {
					...
					hryCore.CURD({
						url : HRY.modules.web + "coin/appicocoin/remove/"+ ids
