...
define([ 'app', 'hryTable', 'layer' ], function(app, DT, layer) {
	...
	function controller($scope, $rootScope, $http, $stateParams, $state, hryCore) {
		...
		if ($stateParams.page == "see") {
			$http.get(HRY.modules.ico + "project/appicoprojectshare/see/" + $stateParams.id).success(function(data) {
				$scope.formData = data;
			});
		}
		...
		if ($stateParams.page == "add") {
			...
			$scope.processForm = function() {
				$http({
					method : 'POST',
					url : HRY.modules.ico + 'project/appicoprojectshare/add',
					...
					if (data.success) {
						...
						window.location.href = '#/ico/project/appicoprojectshare/list/anon';
					} else {
		...
		if ($stateParams.page == "modify") {
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.ico + 'project/appicoprojectshare/modify',
					data : $.param($scope.formData),
					headers : {
						'Content-Type' : 'application/x-www-form-urlencoded'
					}
		...
		if ($stateParams.page == "list") {
			...
			config.ajax.url = HRY.modules.ico + 'project/appicoprojectshare/list';
			...
			$scope.fnSee = function(){
				...
				if (ids.length == 0) {
					...
					window.location.href = "#/ico/project/appicoprojectshare/see/" + ids[0];
				}
			}
			...
			$scope.fnAdd = function(){
				window.location.href = "#/ico/project/appicoprojectshare/add/anon";
			}
			...
			$scope.fnModify = function(){
            	...
            	if (ids.length == 0) {
        			...
        			window.location.href='#/ico/project/appicoprojectshare/modify/'+ids[0];
            	}
			}
			...
			$scope.fnRemove = function() {
				...
				layer.confirm("你确定删除？", {
					...
					hryCore.CURD({
						url : HRY.modules.web + "project/appicoprojectshare/remove/"+ ids
