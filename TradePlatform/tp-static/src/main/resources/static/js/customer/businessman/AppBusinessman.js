...
define([ 'app', 'hryTable', 'layer' ], function(app, DT, layer) {
	...
	function controller($scope, $rootScope, $http, $stateParams, $state, hryCore) {
		...
		if ($stateParams.page == "see") {
			$http.get(HRY.modules.customer + "businessman/appbusinessman/see/" + $stateParams.id).success(function(data) {
				$scope.formData = data;
			});
		}
		...
		if ($stateParams.page == "add") {
			...
			$scope.processForm = function() {
				...
				$scope.formData.feeType = $("#feeType").val();
				$http({
					method : 'POST',
					url : HRY.modules.customer + 'businessman/appbusinessman/add',
					...
					if (data.success) {
						...
						window.location.href = '#/customer/businessman/appbusinessman/list/anon';
					} else {
		...
		if ($stateParams.page == "modify") {
			...
			$http.get(HRY.modules.customer + "businessman/appbusinessman/see/" + $stateParams.id).success(function(data) {
				$scope.formData = data;
				hryCore.RenderSelect($("#type"),data.type);
				hryCore.RenderSelect($("#feeType"),data.feeType);
...
// 	            hryCore.RenderHTML(data);//异步回显控件
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.customer + 'businessman/appbusinessman/modify',
					data : $.param($scope.formData),
					headers : {
						'Content-Type' : 'application/x-www-form-urlencoded'
					}
		...
		if ($stateParams.page == "list") {
			...
			config.ajax.url = HRY.modules.customer + 'businessman/appbusinessman/list';
			...
			$scope.fnSee = function(){
				...
				if (ids.length == 0) {
					...
					window.location.href = "#/customer/businessman/appbusinessman/see/" + ids[0];
				}
			}
			...
			$scope.fnAdd = function(){
				window.location.href = "#/customer/businessman/appbusinessman/add/anon";
			}
			...
			$scope.fnModify = function(){
            	...
            	if (ids.length == 0) {
        			...
        			window.location.href='#/customer/businessman/appbusinessman/modify/'+ids[0];
            	}
			}
			...
			$scope.fnRemove = function() {
				...
				layer.confirm("你确定删除？", {
					...
					hryCore.CURD({
						url : HRY.modules.web + "businessman/appbusinessman/remove/"+ ids
