...
define([ 'app', 'hryTable', 'layer' ], function(app, DT, layer) {
	...
	function controller($scope, $rootScope, $http, $stateParams, $state, hryCore) {
		...
		if ($stateParams.page == "see") {
			$http.get(HRY.modules.customer + "businessman/c2ccoin/see/" + $stateParams.id).success(function(data) {
				$scope.formData = data;
			});
		}
		...
		if ($stateParams.page == "add") {
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.customer + 'businessman/c2ccoin/add',
					...
					if (data.success) {
						...
						window.location.href = '#/customer/businessman/c2ccoin/list/anon';
					} else {
		...
		if ($stateParams.page == "modify") {
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.customer + 'businessman/c2ccoin/modify',
					data : $.param($scope.formData),
					headers : {
						'Content-Type' : 'application/x-www-form-urlencoded'
					}
		...
		if ($stateParams.page == "list") {
			...
			config.ajax.url = HRY.modules.customer + 'businessman/c2ccoin/list';
			...
			$scope.fnAdd = function(){
				window.location.href = "#/customer/businessman/c2ccoin/add/anon";
			}
			...
			$scope.fnModify = function(){
            	...
            	if (ids.length == 0) {
        			...
        			window.location.href='#/customer/businessman/c2ccoin/modify/'+ids[0];
            	}
			}
