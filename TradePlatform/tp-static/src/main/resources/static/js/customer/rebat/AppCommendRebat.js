...
define([ 'app', 'hryTable', 'layer', "hryUtil",'pikadayJq' ], function(app, DT, layer) {
	...
	function controller($scope, $rootScope, $http, $stateParams, $state, hryCore) {
		...
		if ($stateParams.page == "see") {
			$http.get(HRY.modules.customer + "rebat/appcommendrebat/see/" + $stateParams.id).success(function(data) {
				$scope.formData = data;
			});
		}
		...
		if ($stateParams.page == "add") {
			...
			$scope.processForm = function() {
				$http({
					method : 'POST',
					url : HRY.modules.customer + 'rebat/appcommendrebat/add',
					...
					if (data.success) {
						...
						window.location.href = '#/customer/rebat/appcommendrebat/list/anon';
					} else {
		...
		if ($stateParams.page == "modify") {
			...
			$scope.processForm = function() {
				...
				$http({
					method : 'POST',
					url : HRY.modules.customer + 'rebat/appcommendrebat/modify',
					data : $.param($scope.formData),
					headers : {
						'Content-Type' : 'application/x-www-form-urlencoded'
					}
		...
		if ($stateParams.page == "list") {
			...
			config.ajax.url = HRY.modules.customer + 'rebat/appcommendrebat/list';
			...
			config.columnDefs = [ 
				...
				"orderable" : false,
				"render" : function(data, type, row) {
					return data==0?"<front color='red'>失败</front>":data==1?"<front color='green'>成功</front>":"未知";
				},
			}]
			// --------------------加载dataTable--------------------------------
            ...
            $scope.fnExcel = function() {
                DT.excel(table, this.serchData, "返佣派发记录");
            }
			...
			$scope.fnSee = function(){
				...
				if (ids.length == 0) {
					...
					window.location.href = "#/customer/rebat/appcommendrebat/see/" + ids[0];
				}
			}
			...
			$scope.fnAdd = function(){
				window.location.href = "#/customer/rebat/appcommendrebat/add/anon";
			}
			...
			$scope.fnModify = function(){
            	...
            	if (ids.length == 0) {
        			...
        			window.location.href='#/customer/rebat/appcommendrebat/modify/'+ids[0];
            	}
			}
			...
			$scope.fnRemove = function() {
				...
				layer.confirm("你确定删除？", {
					...
					hryCore.CURD({
						url : HRY.modules.web + "rebat/appcommendrebat/remove/"+ ids
