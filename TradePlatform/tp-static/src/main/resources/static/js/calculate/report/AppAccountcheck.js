...
define(['app','hryTable' ,'hryUtil'], function (app, DT) {
    ...
    function controller($scope,$rootScope,$http,$stateParams,hryCore){
    	...
    	var table =  $("#table2");
    		...
    		$http.get(HRY.modules.calculate+"/appReportSettlement/culAccountByCustomerErrorAndRightInfosureold/"+ids).
            ...
            function funddetial1(customerId){
            	  debugger
	          		$http.get(HRY.modules.calculate+"/appReportSettlement/culAccountByCustomerErrorAndRightInfosureold/"+customerId).
	              	success(function(data) {
        ...
        if($stateParams.page=="allerrorinfo"){
     			...
     			layer.confirm('你确定要全部余额核算到数据库吗?请慎重!', {
         			...
         			layer.confirm('你确定要全部余额核算到数据库吗?请慎重!请慎重!', {
                 		...
                 		var index = layer.load(0, {shade: false}); //0代表加载的风格，支持0-2
 		            		$http.get(HRY.modules.calculate+"/appReportSettlement/culAccountAllCustomersureold").
 			            	success(function(data) {
 			            		layer.close(index);
 			            		// 提示信息
        	...
        	if($stateParams.id=="anon"){
        		...
        		var ids=$stateParams.id;
        		$scope.stateParams="ids";
        		$http.get(HRY.modules.calculate+"/appReportSettlement/culAccountByCustomerErrorInfosureold/"+ids).
            	success(function(data) {
            		$scope.data=data;
            		if($scope.data.length==0){
              ...
              function funddetial(customerId){
	          		...
	          		$http.get(HRY.modules.calculate+"/appReportSettlement/culAccountByCustomerErrorInfosureold/"+customerId).
	              	success(function(data) {
	              		debugger
	              		$scope.hisorylist=data[0].hisorylist;
	              		$scope.sureoldlist=data[0].sureoldlist;
          ...
          //导出excel
            ...
            function fnaccounting(){
    			...
    			layer.confirm('你确定要余额核算到数据库吗?请慎重!', {
    				...
    				layer.closeAll();	
        		$http.get(HRY.modules.calculate+"/appReportSettlement/culAccountByCustomersureold/"+customerIds).
            	success(function(data) {
            ...
            function allfnaccountingerrorTomongo(url){
		        		...
		        		$http({
		                    method:"get",
		                    url:HRY.modules.calculate+"/appReportSettlement/culSureOldAccountAllCustomerErrorInfoToRedis?days="+$scope.days,
		                    timeout:500000}).
		        		success(function(data) {
		            		layer.close(index);
		            		$scope.data=data;
            ...
            function fnaccountingerrorAndRight(){
            	...
            	window.location.href='#/calculate/report/appaccountcheck/allerrorrightinfo/'+customerIds;
