 ...
 * 
 * //引入 hrytree define(['app','hrytree'], function (app) { //注入 hrytree
 * app.controller.$inject = ['hrytree']; function controller(hrytree){
 * 
 * function loadDictionaryList(typeid){
 * hrytree.conf.dataurl=HRY.modules.web+"dictionary/appdicmultilevel/findlist.do?rootKey="+typeid;
 * hrytree.conf.fnModify=fnModify; hrytree.conf.fnAdd=fnAdd;
 * hrytree.conf.fnRemove=fnRemove; hrytree.conf.check=true; //是否复选框 // "tree"
 * 为html 中 id="tree" 的值 同一个页面可以 支持多个树 只要 如 id="tree1" ,id="tree2" ...
 * hrytree.init("tree"); }
 */
define([ "app", "layer", "ztree" ], function(app, layer) {

	app.factory('hrytree', [ '$http', '$q', '$resource', function($http, $q, $resource, $scope) {
		...
		function config() {
			...
			function addHandle(treeId, treeNode) {
				...
				if (conf.sort) {
					sObj.after(addStr + editStr + removeStr + upStr + downStr);
				} else {
