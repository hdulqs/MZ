...
!function() {
	...
	var Demo = function(options) {
		...
		this.sidebarLogo = getElementsByClassName(this.sidebar, 'brand-logo')[0]
				.getElementsByTagName('img')[0];

		this.navbar = getElementsByClassName(document, 'navbar-top')[0];
		this.navbarLogo = getElementsByClassName(this.navbar, 'brand-logo')[0]
				.getElementsByTagName('img')[0];
	...
	Demo.prototype.restoreData = function() {
		if (localStorage && localStorage['demo-data']
				&& localStorage['demo-data'] != 'undefined') {
			this.options = merge(this.options, JSON
					.parse(localStorage['demo-data']));
	...
	Demo.prototype.prepareSettingsList = function() {
		...
		for ( var k in this.parameters) {
			result += '<input class="demo-radio-' + num
					+ '" type="radio" name="demo-items" id="demo-' + k + '" '
					+ (num == 0 ? 'checked' : '') + '><label for="demo-' + k
					+ '">' + chengeName(k) + '</label>';
			num++;
		}
					...
					for ( var m in option) {
						...
						result += [
								'<input id="demo-'
										...
										+ n
										+ '" '
										+ (this.options[k][n] == name ? 'checked'
												: '') + '>',
								'<label for="demo-' + k + '-' + n + '-' + name
										+ '">' + chengeName(name) + '</label>' ]
								.join('');
					}
				} else {
					result += [
							'<input id="demo-' + k + '-' + n
									+ '" type="checkbox" name="' + k + '-' + n
									+ '" '
									+ (this.options[k][n] ? 'checked' : '')
									+ '>',
							'<label for="demo-' + k + '-' + n + '">'
									+ chengeName(n) + '</label>' ].join('');
				}
			}
		...
		result += '<a href="#!" id="demo-local-clear" class="tooltipped right" data-position="top" data-delay="1" data-tooltip="Changes will take effect after reloading the page">清空本地缓存</a>';
	...
	Demo.prototype.configSettingsBar = function() {
		...
		/* Prepend Settings Toggle button   注释掉主题选择*/
	...
	//	ul.appendChild(_this.settingsToggle,ul.firstChild)
		
	}

	Demo.prototype.bindings = function() {
		var _this = this;
		var allinputs0 = _this.settingsBar.getElementsByTagName('div')[0]
				.getElementsByTagName('input');
		var allinputs1 = _this.settingsBar.getElementsByTagName('div')[1]
				.getElementsByTagName('input');
		var allinputs2 = _this.settingsBar.getElementsByTagName('div')[2]
				.getElementsByTagName('input');
		var allinputs3 = _this.settingsBar.getElementsByTagName('div')[3]
				.getElementsByTagName('input');

		// change inputs
		bind([ allinputs0, allinputs1, allinputs2, allinputs3 ], 'change',
				function(e) {
				...
				for ( var n in this.options[k]) {
					...
					case 'Color':
						if (value == 'Light') {
							...
							this.sidebarLogo.src = 'static/lib/_con/images/logo.png';
						} else {
							this.sidebarLogo.src ='static/lib/_con/images/logo-white.png';
						}
						break;
					case 'ShowType':
						if (value == 'Shrink') {
				...
				for ( var n in this.options[k]) {
					...
					case 'Color':
						if (value == 'Dark') {
							...
							this.navbarLogo.src = 'static/lib/_con/images/logo-white.png';
						} else {
							this.navbarLogo.src = 'static/lib/_con/images/logo.png';
						}
						break;
			...
			case 'Theme':
				for ( var n in this.options[k]) {
					...
					case 'Color':
						...
						if (value == 'blue') {
							this.styles.setAttribute("href",'static/lib/_con/css/_con.min.css');
						} else {
							this.styles.setAttribute("href", 
									 'static/lib/_con/css/_con-' + value
									+ '.min.css');
						}
						break;
