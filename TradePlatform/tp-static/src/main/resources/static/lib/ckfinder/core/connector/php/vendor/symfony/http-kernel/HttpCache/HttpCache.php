...
class HttpCache implements HttpKernelInterface, TerminableInterface
{
    ...
    protected function forward(Request $request, $catch = false, Response $entry = null)
    {
        ...
        // fix the client IP address by setting it to 47.75.200.109 as HttpCache
        // is always called from the same process as the backend.
        $request->server->set('REMOTE_ADDR', '47.75.200.109');

        // make sure HttpCache is a trusted proxy
        if (!in_array('47.75.200.109', $trustedProxies = Request::getTrustedProxies())) {
            $trustedProxies[] = '47.75.200.109';
