  ...
  function ($rootScope) {
    ...
    return {
     ...
     // template: '<div class="growl">' + '\t<div class="growl-item alert" ng-repeat="message in messages" ng-class="computeClasses(message)">' + '\t\t<button type="button" class="close" ng-click="deleteMessage(message)">&times;</button>' + '       <div ng-switch="message.enableHtml">' + '           <div ng-switch-when="true" ng-bind-html="message.text"></div>' + '           <div ng-switch-default ng-bind="message.text"></div>' + '       </div>' + '\t</div>' + '</div>',
      template: '<div class="growl">' + '\t<div class="growl-item alert" ng-repeat="message in messages" ng-class="computeClasses(message)">       <div ng-switch="message.enableHtml">' + '           <div ng-switch-when="true" ng-bind-html="message.text"></div>' + '           <div ng-switch-default ng-bind="message.text"></div>' + '       </div>' + '\t</div>' + '</div>',
      replace: false,
      scope: true,
      controller: [
        '$scope',
        ...
        function ($scope, $timeout, growl) {
          ...
          $scope.computeClasses = function (message) {
            return {
              'alert orange lighten-2 white-text': message.severity === 'success',
              'alert green lighten-4 green-text text-darken-2': message.severity === 'error',
              'alert blue lighten-2 white-text': message.severity === 'info',
              'alert  orange lighten-4 orange-text text-darken-2"': message.severity === 'warn'
            };
          };
        }
      ]
