...
public class HtlRpcHttpClient {

  private static String protocol = Properties
      .appcoinMap().get(Constant.HTL.toLowerCase() + "_protocol");
  private static String ip = Properties.appcoinMap()
      .get(Constant.HTL.toLowerCase() + "_ip");
  private static String port = Properties.appcoinMap()
      .get(Constant.HTL.toLowerCase() + "_port");

  public static String getHttpClient(String methodname, List list) {
    String url = "";
    try {
      url =
          protocol + "://" + ip + ":" + port + "?jsonrpc=2.0&id=1&method=" + methodname + "&params="
