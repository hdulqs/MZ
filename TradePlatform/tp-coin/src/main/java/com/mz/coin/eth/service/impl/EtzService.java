 ...
 */
public class EtzService {
	// 配置信息
	private static String coinType = Constant.ETZ;
	...
	public static String createAddress() {
		...
		try {
			...
			String ethAccountsStr = RedisUtil.getValue(ACCOUNTS);
			if (StringUtils.isNotEmpty(address) && StringUtils.isNotEmpty(ethAccountsStr)) {
				List<String> eth_accounts = JSON.parseArray(ethAccountsStr, String.class);
			...
			System.out.println("获取account:" + account);
			System.out.println("获取address:" + address);
			e.printStackTrace();
		}
		return address;
	}
	...
	public static String getBasecoin() {
		...
		try {
			...
			LogFactory.info(coinType + "-getBasecoin 钱包接口不通");
		} catch (IOException e) {
			e.printStackTrace();
		} catch (NullPointerException e) {
			e.printStackTrace();
	...
	public static JsonResult sendFrom(String amount, String toAddress) {
		...
		if (fromAddressMoney != null && fromAddressMoney.compareTo(needfunds) >= 0) {
			...
			result.setMsg("ETC提币账户余额不足,至少需要" + Convert.fromWei(new BigDecimal(needfunds.toString()), Unit.ETHER));
		}
		return result;
	}
	...
	public static void productionTx() {
		// 可能会漏掉，需要再次考虑逻辑
		long n = 15L;
		BigInteger blockNumber = getBlockNumber().subtract(BigInteger.valueOf(n));
	...
	public static void recordGethTx(org.web3j.protocol.core.methods.response.Transaction tx) {
		if (tx != null) {
			...
			String accountsStr = RedisUtil.getValue(ACCOUNTS);
			if (to != null && accountsStr.contains(to) && txService.existNumber(tx.getHash()) == 0&&tx.getValue()!=null&&BigInteger.ZERO.compareTo(tx.getValue())<0) {
	...
	public static void recordEtcTx(org.web3j.protocol.core.methods.response.Transaction tx) {
		if (tx != null) {
			...
			if (to != null && ACCOUNTS_str.contains(to) && txService.existNumber(tx.getHash()) == 0) {// 以太经典
				AppCoinTransaction t = new AppCoinTransaction();
				t.setHash_(tx.getHash());
				t.setTransactionIndex(tx.getTransactionIndex().toString());
				t.setBlockNumber(tx.getBlockNumber().toString());
