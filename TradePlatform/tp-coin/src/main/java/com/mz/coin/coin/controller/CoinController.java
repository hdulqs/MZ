...
public class CoinController {
  ...
  public Map<String, String> create(HttpServletRequest req) {
    ...
    String address = null;
    if (StringUtils.isNotEmpty(userName) && StringUtils.isNotEmpty(type) && StringUtils
      ...
      try {
        ...
        } else if (type.equalsIgnoreCase(Constant.ETHER) || EtherService
            .isSmartContractCoin(type)) {
      ...
      LogFactory.info("创建币地址参数信息：userName=" + userName + "    type=" + type + "      accountNumber="
          + accountNumber);
      map.put("address", address);
  ...
  public JsonResult sendFrom(HttpServletRequest request) {
    ...
    System.out.println(
        "转账接口：toAddress:" + toAddress + ",type:" + type + ",transactionNum:" + transactionNum
            + ",amount:" + amount + ",auth_code:" + getcode + ",keepDecimalForCoin:"
            + keepDecimalForCoin + ",memo:" + memo);
      ...
      if (StringUtils.isNotEmpty(authcode) && getcode.equals(authcode)) {
        if (StringUtils.isNotEmpty(type)) {
          ...
          if (type.equalsIgnoreCase(Constant.ETC)) {
            ...
            result = client
                .omniSendFrom(amount, toAddress, Integer.parseInt(Constant.PROPERTYID_USDT));
          } else if (type.equalsIgnoreCase(Constant.BDS)) {
            BdsServerImpl bdsServerImpl = new BdsServerImpl();
...
//						continue;
      ...
      if (StringUtils.isNotEmpty(toAddress)) {
        if (isValidAddress) {
          if (type.equalsIgnoreCase(Constant.ETC)) {
            ...
            JsonrpcClient client = MyCoinService.getClient(type);
            result = client
                .omnisendColdWallet(amount, toAddress, Integer.parseInt(Constant.PROPERTYID_USDT));
    ...
    try {
      ...
      if (null != count && !"".equals(count)) {
        ...
        if (null != startWith && !"".equals(startWith)) {
          list = coinService
              .listTransactions(userName, Integer.valueOf(count), Integer.valueOf(startWith));
          result = JSON.stringify(list);
        } else {
          list = coinService.listTransactions(userName, Integer.valueOf(count));
          result = JSON.stringify(list);
  ...
  public JsonResult refreshUserCoin(HttpServletRequest request) {
    ...
    ExDigitalmoneyAccountService accountService = (ExDigitalmoneyAccountService) ContextUtil
        .getBean("exDigitalmoneyAccountService");
    ExDigitalmoneyAccount coinAccount = accountService.getAccountByAccountNumber(account);
    if (StringUtils.isNotEmpty(coinCode) && StringUtils.isNotEmpty(account) && StringUtils
        .isNotEmpty(countstr)) {
      String[] arrcoin = Properties.listCoinBasedBtc();// 基于比特币币种类型
      if (arrcoin != null && arrcoin.length > 0 && Json.toJson(arrcoin)
          .contains(coinCode.toUpperCase())) {
        CoinTransactionService txService = (CoinTransactionService) ContextUtil
            .getBean("coinTransactionService");
  ...
  public JsonResult rechargeTxFee2TokenAddress(HttpServletRequest request) {
    ...
    if (CommonUtil.isNohashEmptyInArr(params) && checkcode.equals(authcode)) {
      ...
      if (EtherService.isSmartContractCoin(coinType)) {
        ...
        JsonrpcClient client = MyCoinService.getClient(Constant.USDT);
        result = client
            .omniRechargeTxFee(address, amount, Integer.parseInt(Constant.PROPERTYID_USDT));
      }
    } else {
  ...
  public JsonResult tokenCollect(HttpServletRequest request) {
    ...
    if (StringUtils.isNotEmpty(address) && StringUtils.isNotEmpty(coinType)) {
      if (Constant.USDT.equalsIgnoreCase(coinType)) {
        JsonrpcClient client = MyCoinService.getClient(Constant.USDT);
        result = client
            .omniAssetCollect(address, amount, Integer.parseInt(Constant.PROPERTYID_USDT));
      } else {
