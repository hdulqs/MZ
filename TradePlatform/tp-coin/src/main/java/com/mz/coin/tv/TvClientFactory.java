 ...
 * <p> TODO</p>
 * @author:         shangxl
 * @Date :          2017年3月14日 上午9:44:51
 */
public class TvClientFactory {

    private volatile static TvClientFactory btsClientFactory;
    private TvClientFactory(){};
...
//                System.out.println("返回报文信息："+returnMessage);
                JSONObject returnObject = JSONObject.parseObject(returnMessage);
                result = returnObject.toJSONString();

				/*if (returnObject.containsKey("result")){
					result = returnObject.getString("result");
				}else {
					result = returnObject.getString("error");
				}*/
            };
        } catch (IOException e) {
//            e.printStackTrace();
