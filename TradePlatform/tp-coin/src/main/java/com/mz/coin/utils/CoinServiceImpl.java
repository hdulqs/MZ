	...
	public static BitcoinJSONRPCClient getClient(String type) {
		...
		if (mapClient != null && mapClient.get(type) != null) {
			...
			if (mapClient == null || mapClient.get(type) == null) {
				...
				if(!StringUtils.isNotEmpty(ip)||!StringUtils.isNotEmpty(port)||!StringUtils.isNotEmpty(rpcuser)||!StringUtils.isNotEmpty(rpcpassword)||!StringUtils.isNotEmpty(protocol)) {
					LogFactory.error(type+"配置文件为空");
					return null;
				}
				//模仿telnet
//				if(HttpConnectionUtil.isHostConnectable(ip, Integer.valueOf(port))){
...
//			e.printStackTrace();
	...
	public String sendFrom(String fromAddress, String toBitcoinAddress, double amount) {
		...
		try {
			...
			if (map.get(isvalid).toString()!=null&&Boolean.parseBoolean(map.get(isvalid).toString())) {
...
//			e.printStackTrace();
	...
	public Wallet getWalletInfo(String coinCode) {
		...
		if (balance != null) {
			...
			String coldwalletAddress=Properties.appcoinMap().get(coinCode.toLowerCase() + Properties.COLDADDERSS);
