...
public class DrawPictureUtil {
  ...
  public void darw(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
    // 生成一张图片
     ...
     * 将验证码存入session中
     */
    session.setAttribute(codeName, checkcode);
    log.info("图形验证码为:" + codeName + "|" + session.getAttribute(codeName));
    // 将图片写到浏览器上
