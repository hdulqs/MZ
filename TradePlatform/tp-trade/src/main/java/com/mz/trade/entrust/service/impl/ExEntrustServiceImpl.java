...
public class ExEntrustServiceImpl extends BaseServiceImpl<ExEntrust, Long> implements
      ...
      if (null != buyList && buyList.size() > 0) {
        ...
        for (ExEntrust buyExEntrust : buyList) {
          ...
          if (buyExEntrust.getEntrustPrice().compareTo(sameprice) == 0) {
            ...
            String key =
                header + ":buy:" + sameprice.setScale(10, BigDecimal.ROUND_HALF_EVEN).toString();
      ...
      if (null != sellList && sellList.size() > 0) {
        ...
        for (ExEntrust sellExEntrust : sellList) {
          ...
          if (sellExEntrust.getEntrustPrice().compareTo(entrustPrice) == 0) {
            ...
            String key = header + ":sell:" + entrustPrice.setScale(10, BigDecimal.ROUND_HALF_EVEN)
                .toString();
        ...
        String key =
            header + ":sell:" + entrustPrice.setScale(10, BigDecimal.ROUND_HALF_EVEN).toString();
        redisTradeService.save(key, JSON.toJSONString(listsellpricesame));
    ...
    if (!StringUtils.isEmpty(str)) {
      ...
      if (coins != null && coins.size() > 0) {
        for (CoinKeepDecimal coin : coins) {
          if (coinCode.equals(coin.getCoinCode()) && fixPriceCoinCode
   ...
   */
  private static List<EntrustTrade> getMatchEntrustTradebyType(ExCointoCoin exCointoCoin,
      Long customerId, int entrustType) {
    ...
    if (exCointoCoin != null && entrustType != 0) {
      ...
      if (entrustType == 1) {
        String buyKey =
            exCointoCoin.getCoinCode() + "_" + exCointoCoin.getFixPriceCoinCode() + ":buy";
        keys = redisTradeService.noPerkeys(buyKey + ":");
      } else if (entrustType == 2) {
        String sellKey =
            exCointoCoin.getCoinCode() + "_" + exCointoCoin.getFixPriceCoinCode() + ":sell";
  ...
  private BigDecimal getHuobiReferencePriceByApi(String coinCouple) throws ApiException {
    ...
    ObjectMapper objectMapper = new ObjectMapper();
    List<HistoryTrade> historyTradeList = objectMapper
        .convertValue(tradeResponse.getTick().getData(),
   ...
   */
  @Override
  public boolean addExEntrust(String coinCode, String fixPriceCoinCode, Integer fixPriceType,
      Integer type, Integer entrustWay, Long customerId, BigDecimal price, String autoUsername,
      BigDecimal entrustCount, String currencyType, String website) {
    EntrustTrade entrustTrade = new EntrustTrade();
    ...
    if (entrustWay != null) {
      if (entrustWay == 1) {
        ...
        entrustTrade
            .setEntrustPrice(price.setScale(keepDecimalForCurrency, BigDecimal.ROUND_HALF_DOWN));
        entrustTrade
            .setEntrustCount(entrustCount.setScale(keepDecimalForCoin, BigDecimal.ROUND_HALF_DOWN));
      } else {
        entrustTrade.setEntrustWay(entrustWay);// 1.限价--> 表示以固定的价格 , 2.市价--->
        if (type == 1) {
          entrustTrade.setEntrustSum(price.multiply(entrustCount)
              .setScale(keepDecimalForCurrency, BigDecimal.ROUND_HALF_DOWN));
        } else {
          entrustTrade.setEntrustCount(
              entrustCount.setScale(keepDecimalForCoin, BigDecimal.ROUND_HALF_DOWN));
        }
      }
    }
  ...
  public void addHedgeEntrustByAutoAccount(ExCointoCoin exCointoCoin, Integer entrustType,
      String currencyType, String website) {
    if (exCointoCoin == null || exCointoCoin.getIsHedge() == null
        || exCointoCoin.getIsHedge() != 1) {
      return;
    }
    String coinCouple = exCointoCoin.getCoinCode() + "_" + exCointoCoin.getFixPriceCoinCode();
    String autoUsernames = exCointoCoin.getAutoUsername();
    ...
    for (String autoUsername : autoUsernameArr) {
      ...
      for (int i = 0; i < 2; i++) {
        List<EntrustTrade> entrustTradeList = getMatchEntrustTradebyType(exCointoCoin,
            exCointoCoin.getCustomerId(), entrustType);
      ...
      if (entrustNum == null || entrustNum.compareTo(BigDecimal.ZERO) <= 0) {
        entrustNum = getFloatNum(exCointoCoin.getAutoCount(), exCointoCoin.getAutoCountFloat(),
            null);
      }
   ...
   */
  public void addEntrustByAutoAccount(ExCointoCoin exCointoCoin, Integer entrustType,
      String currencyType, String website) {
    String autoUsernames = exCointoCoin.getAutoUsername();
    if (autoUsernames == null) {
      return;
    ...
    for (String autoUsername : autoUsernameArr) {
      ...
      BigDecimal autoPrice = getPrcie(exCointoCoin.getAtuoPriceType(), exCointoCoin.getUpFloatPer(),
          coinCouple, exCointoCoin.getAutoPrice(), exCointoCoin.getAutoPriceFloat());
      BigDecimal entrustNum = getFloatNum(exCointoCoin.getAutoCount(),
          exCointoCoin.getAutoCountFloat(), null);
      if (!addExEntrust(exCointoCoin.getCoinCode(), exCointoCoin.getFixPriceCoinCode(),
          exCointoCoin.getFixPriceType(),
  ...
  public void initExEntrust(EntrustTrade exEntrust) {
    ...
    if (userRedis == null) {
      ...
      exEntrust.setTransactionSum(null == exEntrust.getTransactionSum() ? new BigDecimal("0")
          : exEntrust.getTransactionSum());
      exEntrust.setEntrustSum(
          (null != exEntrust.getEntrustPrice() && null != exEntrust.getEntrustCount())
              && !new BigDecimal("0").equals(exEntrust.getEntrustPrice()) && !new BigDecimal("0")
              .equals(exEntrust.getEntrustCount()) ? exEntrust.getEntrustPrice()
              .multiply(exEntrust.getEntrustCount()) : exEntrust.getEntrustSum());
  ...
  /**
   * 根据浮动比例获取浮动值 当@param autoPriceType大于3的时候，就是获取参考价格{@link #refreshReferencePrice}
   *
   * @param autoPriceType 价格类型，1定价下单，2市价下单，3参考火币价格
   */
  public BigDecimal getPrcie(Integer autoPriceType, BigDecimal upFloatPer, String coinCouple,
      BigDecimal basePrice, BigDecimal floatPrice) {
    ...
    switch (autoPriceType) {
      case 1:     // 按定价浮动
        break;
      case 2: {   // 按市价浮动
        String currentPrice = TradeRedis
            .getStringData(coinCouple + ":" + ExchangeDataCacheRedis.CurrentExchangPrice);
      ...
      case 3: { // 获取参考价格
        String currentPrice = TradeRedis
            .getStringData(coinCouple + ":" + ExchangeDataCacheRedis.ReferenceExchangPrice);
        if (!StringUtil.isEmpty(currentPrice)) {
  ...
  /**
   * 根据基准价格和浮动比率获取一个浮动价格 获取浮动值 刷币价格 * (浮动比例 * 随机小数 )
   *
  ...
  public String getExEntrustChangeMarket(String coinCode, String fixPriceCoinCode, Integer n) {
    ...
    if (keyslistbuy.size() > 0) {
      String keybuy =
          TradeRedis.getHeaderFront(sellexEntrust) + ":" + ExchangeDataCacheRedis.BuyOnePrice;
    ...
    if (keyslistsell.size() > 0) {
      String keysell =
          TradeRedis.getHeaderFront(byllexEntrust) + ":" + ExchangeDataCacheRedis.SellOnePrice;
  ...
  @Override
  public String getExEntrustChangeDephMarket(String coinCode, String fixPriceCoinCode, Integer n,
      BigDecimal jj) {
    MarketDepths marketDepths = new MarketDepths();
    Map<String, List<BigDecimal[]>> map = new HashMap<String, List<BigDecimal[]>>();
    int keepDecimalForCoin = 4;
    ...
    BigDecimal depth = new BigDecimal(1)
        .divide(getByn(keepDecimalForCurrencysubone), keepDecimalForCurrencysubone,
              ...
              if (null != list) {
                for (EntrustTrade entrustTrade : list) {
                  surplusEntrustCount = surplusEntrustCount
                      .add(entrustTrade.getSurplusEntrustCount());
                }
              }
              i++;
            } else {
